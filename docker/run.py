...
# Services that get `${REPO}:/mnt/src` mounted with tmpfs over /mnt/src/.git.
# Worktrees have `.git` as a *file* (gitdir pointer) so the tmpfs mount fails.
SERVICE_TMPFS_KEEP = {
    "api-v1": ["/mnt/openreview-py/.git"],
    "api-v2": ["/mnt/openreview-py/.git"],
    "web": [],
}
...
def write_worktree_override(worktree_services):
    """Write a compose override that strips /mnt/src/.git tmpfs for worktree-bound services.
...
def main():
    ...
    worktree_services = []
    if auto_checkout:
        api_v1_path, used_wt = checkout_branch(api_v1_path, api_v1_branch, "api-v1")
        if used_wt:
            worktree_services.append("api-v1")
        api_v2_path, used_wt = checkout_branch(api_v2_path, api_v2_branch, "api-v2")
        if used_wt:
            worktree_services.append("api-v2")
        web_path, used_wt = checkout_branch(web_path, web_branch, "web")
        if used_wt:
            worktree_services.append("web")

    override = write_worktree_override(worktree_services)
