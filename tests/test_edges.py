...
class TestEdges:
    ...
    def test_rename_edges(self, client):
        ...
        to_profile = guest.register_user(email = 'lbahy@mail.com', first = 'Nadia', last = 'LBahy', password = '1234')
        assert to_profile
        assert to_profile['id'] == '~Nadia_LBahy1'
        super_user_edges = list(openreview.tools.iterget_edges(client, tail="~Super_User1"))
        client.rename_edges('~Super_User1', '~Nadia_LBahy1')
        nadias_edges = list(openreview.tools.iterget_edges(client, tail="~Nadia_LBahy1"))
        ...
        client.rename_edges('~Nadia_LBahy1','~Super_User1')
