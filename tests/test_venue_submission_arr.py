...
class TestVenueSubmissionARR():
    ...
    def test_meta_review_stage(self, venue, openreview_client, helpers):
        ...
        assert openreview_client.get_invitation('ARR/-/Meta_Review')
        with pytest.raises(openreview.OpenReviewException, match=r'The Invitation ARR/Submission1/-/Meta_Review was not found'):
            assert openreview_client.get_invitation('ARR/Submission1/-/Meta_Review')
        ...
        openreview_client.post_invitation_edit(
            ...
            writers=['ARR'],
            signatures=['ARR'],
            invitation=openreview.api.Invitation(id='ARR/-/Meta_Review',
                cdate=openreview.tools.datetime_millis(datetime.datetime.utcnow()) + 2000,
                signatures=['ARR']
            )
        )

        helpers.await_queue_edit(openreview_client, 'ARR/-/Meta_Review-0-0')
