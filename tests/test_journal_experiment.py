...
class TestJournal():
    ...
    def journal(self, openreview_client, helpers):
        ...
        venue_id = 'TMLRE'
        paul_client=OpenReviewClient(username='pvicol@mail.com', password=helpers.strong_password)
        paul_client.impersonate('TMLRE/Editors_In_Chief')

        requests = openreview_client.get_notes(invitation='openreview.net/Support/-/Journal_Request', content={ 'venue_id': venue_id })

        return JournalRequest.get_journal(paul_client, requests[0].id)
    ...
    def test_setup(self, openreview_client, request_page, selenium, helpers, journal_request):
        ...
        ## Support Role
        helpers.create_user('pvicol@mail.com', 'Paul', 'Vicol')
        ...
        ## External Reviewers
        helpers.create_user('rachel@mailten.com', 'Rachel', 'TMLRE')

        ## Authors
        helpers.create_user('melisa@maileight.com', 'Melisa', 'TMLRE')
        helpers.create_user('ana@mailnine.com', 'Ana Celeste', 'TMLRE')
        ...
        request_form = openreview_client.post_note_edit(invitation= 'openreview.net/Support/-/Journal_Request',
            ...
            note = Note(
                ...
                content = {
                    ...
                    'support_role': {'value': '~Paul_Vicol1' },
                    ...
                    'settings': {
                        'value': {
                            ...
                            'issn': '2835-8856',
                            'website_urls': {
                                'editorial_board': 'https://jmlr.org/tmlre/editorial-board.html',
                                'evaluation_criteria': 'https://jmlr.org/tmlre/editorial-policies.html#evaluation',
                                'reviewer_guide': 'https://jmlr.org/tmlre/reviewer-guide.html',
                                'editorial_policies': 'https://jmlr.org/tmlre/editorial-policies.html',
                                'faq': 'https://jmlr.org/tmlre/contact.html',
                                'videos': 'https://tmlre.infinite-conf.org'                
                            },
                            ...
                            'official_recommendation_additional_fields': {
                                'pilot_recommendation_to_iclr_track': {
                                    'order': 98,
                                    'description': 'Would you recommend this work be invited for presentation at the ICLR Journal-to-Conference Track? Recall that TMLRE\'s acceptance criteria are that a work must be sound and of interest to the TMLRE audience. Above these requirements, a paper in the ICLR Journal-to-Conference Track should also stand out in novelty or predicted significance for the field (i.e., comparable to the level of a paper in ICLR\'s regular conference track). **Your anonymized response will be shared with ICLR**',
                                    'value': {
                                        'param': {
                                    ...
                                    'readers': ['TMLRE', 'TMLRE/Paper${7/content/noteNumber/value}/Action_Editors', '${5/signatures}']
                                },
                            ...
                            'decision_additional_fields': {
                                'pilot_recommendation_to_iclr_track': {
                                    ...
                                    'readers': ['TMLRE', 'TMLRE/Paper${7/content/noteNumber/value}/Action_Editors']
                                },
                                'pilot_explain_recommendation_to_iclr_track': {
                                    'order': 98,
        ...
        openreview_client.add_members_to_group('TMLRE/Expert_Reviewers', ['~Andrew_McCallumm1'])

        TMLRE =  openreview_client.get_group('TMLRE')
        assert TMLRE
        assert TMLRE.members == ['~Paul_Vicol1', 'TMLRE/Editors_In_Chief']
        assert TMLRE.content['submission_id']['value'] == 'TMLRE/-/Submission'
        assert TMLRE.content['certifications']['value'] == ['Featured Certification', 'Reproducibility Certification', 'Survey Certification']
        assert TMLRE.content['eic_certifications']['value'] == ['Outstanding Certification']
        assert TMLRE.content['expert_reviewer_certification']['value'] == 'Expert Certification'
        assert TMLRE.content['event_certifications']['value'] == ['lifelong-ml.cc/CoLLAs/2023/Journal_Track']
        assert TMLRE.content['journal_experiment']['value'] == True

        invitation = openreview_client.get_invitation('TMLRE/-/Accepted')
        ...
        assert openreview_client.get_group('TMLRE/Reviewers/Archived')
        ...
        assert openreview.tools.get_invitation(openreview_client, 'TMLRE/-/Preferred_Emails')
        assert openreview_client.get_edges_count(invitation='TMLRE/-/Preferred_Emails') == 0

    def test_invite_action_editors(self, journal, openreview_client, request_page, selenium, helpers):

        venue_id = 'TMLRE'
        request_notes = openreview_client.get_notes(invitation='openreview.net/Support/-/Journal_Request', content= { 'venue_id': 'TMLRE' })
        ...
        invited_group = openreview_client.get_group('TMLRE/Action_Editors/Invited')
        ...
        for message in messages:
            ...
            assert messages
            assert 'Thank you for accepting this invitation from Transactions on Machine Learning Research Experiment' == messages[0].text


        helpers.await_queue_edit(openreview_client, invitation = 'TMLRE/Action_Editors/-/Recruitment')
        ...
        request_page(selenium, "http://localhost:3030/group?id=TMLRE/Action_Editors", joelle_client.token, wait_for_element='group-container')
    ...
    def test_invite_reviewers(self, journal, openreview_client, request_page, selenium, helpers):
        ...
        invited_group = openreview_client.get_group('TMLRE/Reviewers/Invited')
        ...
        helpers.await_queue_edit(openreview_client, invitation = 'TMLRE/Reviewers/-/Recruitment')
    ...
    def test_submission(self, journal, openreview_client, test_client, helpers):
        ...
        openreview_client.remove_members_from_group('TMLRE/Action_Editors', 'user@mail.com')
        openreview_client.remove_members_from_group('TMLRE/Reviewers', 'zach@mail.com')
        ...
        ## Set a max quota
        david_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Custom_Max_Papers',
            readers=[venue_id, 'TMLRE/Action_Editors', '~David_Belanger1', 'TMLRE/Action_Editors/Archived'],
            writers=[venue_id, '~David_Belanger1'],
            signatures=['~David_Belanger1'],
            head='TMLRE/Reviewers',
        ...
        submission_note_1 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
        ...
        messages = openreview_client.get_messages(to = 'test@mail.com', subject = '[TMLRE] New submission to TMLRE: Paper title')
        ...
        edges = []
        for ae in openreview_client.get_group('TMLRE/Action_Editors').members:
            edges.append(openreview.api.Edge(invitation='TMLRE/Action_Editors/-/Affinity_Score',
                readers=['TMLRE', 'TMLRE/Paper1/Authors', ae],
                writers=['TMLRE'],
                signatures=['TMLRE'],
                head=note_id_1,
                tail=ae,
        ...
        openreview_client.get_invitation('TMLRE/Paper1/Action_Editors/-/Recommendation')

        messages = openreview_client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Suggest candidate Action Editor for your new TMLRE submission')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == '''Hi SomeFirstName User,

Thank you for submitting your work titled "Paper title" to TMLRE.
...
To do so, please follow this link: https://openreview.net/invitation?id=TMLRE/Paper1/Action_Editors/-/Recommendation or check your tasks in the Author Console: https://openreview.net/group?id=TMLRE/Authors

For more details and guidelines on the TMLRE review process, visit jmlr.org/tmlre.

The TMLRE Editors-in-Chief
...
'''
        ...
        assert author_group
        assert author_group.members == ['~SomeFirstName_User1', '~Melisa_TMLRE1', '~Andrew_McCallumm1']
        ...
        assert note
        assert note.invitations == ['TMLRE/-/Submission']
        ...
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1', '~Andrew_McCallumm1']
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Action_Editors/-/Recommendation-0-0')

        messages = journal.client.get_messages(subject = '[TMLRE] You are late in performing a task for your paper 1: Paper title')
        assert len(messages) == 3
        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] You are late in performing a task for your paper 1: Paper title')
...
Our records show that you are late on the current task:
  ...
  Link: https://openreview.net/group?id=TMLRE/Authors#author-tasks
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Action_Editors/-/Recommendation-0-1')
        ...
        ## Update submission 1
        updated_submission_note_1 = test_client.post_note_edit(invitation='TMLRE/Paper1/-/Revision',
            signatures=['TMLRE/Paper1/Authors'],
            note=Note(
                content={
        ...
        messages = openreview_client.get_messages(subject='[TMLRE] Revision posted on submission 1: Paper title UPDATED')
        assert len(messages) == 3

        note = openreview_client.get_note(note_id_1)
        assert note
        assert note.number == 1
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision']
        assert note.readers == ['TMLRE', 'TMLRE/Paper1/Action_Editors', 'TMLRE/Paper1/Authors']
        assert note.writers == ['TMLRE', 'TMLRE/Paper1/Authors']
        assert note.signatures == ['TMLRE/Paper1/Authors']
        ...
        assert note.content['supplementary_material']['readers'] == ["TMLRE", "TMLRE/Paper1/Action_Editors", "TMLRE/Paper1/Reviewers", "TMLRE/Paper1/Authors"]
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1', '~Andrew_McCallumm1']
        assert note.content['authorids']['readers'] == ['TMLRE', 'TMLRE/Paper1/Action_Editors', 'TMLRE/Paper1/Authors']
        ...
        submission_note_2 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
                                    signatures=['~SomeFirstName_User1'],
                                    note=Note(
                                        content={
                                            ...
                                            'authorids': { 'value': ['~SomeFirstName_User1', '~Ana_Celeste_TMLRE1']},
                                            'competing_interests': { 'value': 'None beyond the authors normal conflict of interests'},
                                            'human_subjects_reporting': { 'value': 'Not applicable'},
                                            'pdf': { 'value': '/pdf/22234qweoiuweroi22234qweoiuweroi12345678.pdf' },
                                            'submission_length': { 'value': 'Regular submission (no more than 12 pages of main content)'}
        ...
        openreview_client.get_invitation('TMLRE/Paper2/Action_Editors/-/Recommendation')

        author_group=openreview_client.get_group(f"{venue_id}/Paper2/Authors")
        assert author_group
        assert author_group.members == ['~SomeFirstName_User1', '~Ana_Celeste_TMLRE1']
        ...
        submission_note_3 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
                                    signatures=['~SomeFirstName_User1'],
                                    note=Note(
                                        content={
        ...
        openreview_client.get_invitation('TMLRE/Paper3/Action_Editors/-/Recommendation')        
        ...
        paper_assignment_edge = raia_client.post_edge(openreview.api.Edge(invitation='TMLRE/Action_Editors/-/Assignment',
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Assignment to new TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 0
...
With this email, we request that you manage the review process for a new TMLRE submission "1: Paper title UPDATED".

As a reminder, TMLRE Action Editors (AEs) are **expected to accept all AE requests** to manage submissions that fall within your expertise and quota. Reasonable exceptions are 1) situations where exceptional personal circumstances (e.g. vacation, health problems) render you incapable of fully performing your AE duties or 2) you have a conflict of interest with one of the authors. If any such exception applies to you, contact us at tmlre@jmlr.org.

Your first task is to make sure the submitted preprint is appropriate for TMLRE and respects our submission guidelines. Clear cases of desk rejection include submissions that are not anonymized, submissions that do not use the unmodified TMLRE stylefile and submissions that clearly overlap with work already published in proceedings (or currently under review for publication). If you suspect but are unsure about whether a submission might need to be desk rejected for any other reasons (e.g. lack of fit with the scope of TMLRE or lack of technical depth), please email us.

Please follow this link to perform this task: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Review_Approval

If you think the submission can continue through TMLRE's review process, click the button "Under Review". Otherwise, click on "Desk Reject". Once the submission has been confirmed, then the review process will begin, and your next step will be to assign 3 reviewers to the paper. You will get a follow up email when OpenReview is ready for you to assign these 3 reviewers.

We thank you for your essential contribution to TMLRE!
...
'''
        ...
        invitation = openreview_client.get_invitation(id='TMLRE/Paper1/Action_Editors/-/Recommendation')
        assert invitation.expdate is not None
        assert invitation.expdate < openreview.tools.datetime_millis(datetime.datetime.now())
        assert openreview_client.get_invitation('TMLRE/Paper1/-/Review_Approval')
        ...
        comment_note_edit = joelle_client.post_note_edit(invitation=f'{venue_id}/Paper1/-/Official_Comment',
            ...
            note=Note(
                signatures=[joelle_paper1_anon_group.id],
                readers=['TMLRE/Editors_In_Chief', 'TMLRE/Paper1/Action_Editors'],
                forum=note_id_1,
                replyto=note_id_1,
                content={
        ...
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper1/-/Review_Approval',
                                    signatures=[joelle_paper1_anon_group.id],
                                    note=Note(content={
        ...
        assert note
        assert note.odate
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review']
...
note={Under review}
        ...
        edits = openreview_client.get_note_edits(note.id, invitation='TMLRE/-/Under_Review')
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Perform reviewer assignments for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,

With this email, we request that you assign 3 reviewers to your assigned TMLRE submission "1: Paper title UPDATED". The assignments must be completed **within 1 week** ({(datetime.datetime.now() + datetime.timedelta(weeks = 1)).strftime("%b %d")}). To do so, please follow this link: https://openreview.net/group?id=TMLRE/Action_Editors and click on "Edit Assignment" for that paper in your "Assigned Papers" console.

As a reminder, up to their annual quota of 6 reviews per year, reviewers are expected to review all assigned submissions that fall within their expertise. Acceptable exceptions are 1) if they have an unsubmitted review for another TMLRE submission or 2) situations where exceptional personal circumstances (e.g. vacation, health problems) render them incapable of fully performing their reviewing duties.
...
'''
        ...
        paper_assignment_edge_one = raia_client.post_edge(openreview.api.Edge(invitation='TMLRE/Action_Editors/-/Assignment',
        ...
        helpers.await_queue_edit(openreview_client, invitation='TMLRE/Action_Editors/-/Assignment', count=6)
        ...
        desk_reject_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper2/-/Review_Approval',
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper2/-/Desk_Rejection_Approval-0-0')

        messages = journal.client.get_messages(subject = '[TMLRE] You are late in performing a task for the paper 2: Paper title 2')
        assert len(messages) == 2

        messages = journal.client.get_messages(to='kyunghyun@mail.com', subject = '[TMLRE] You are late in performing a task for the paper 2: Paper title 2')
...
'''
        ...
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper2/-/Desk_Rejection_Approval',
        ...
        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Decision for your TMLRE submission 2: Paper title 2')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,

We are sorry to inform you that, after consideration by the assigned Action Editor, your TMLRE submission "2: Paper title 2" has been rejected without further review.

Cases of desk rejection include submissions that are not anonymized, submissions that do not use the unmodified TMLRE stylefile and submissions that clearly overlap with work already published in proceedings (or currently under review for publication).
...
'''
        ...
        assert note
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Desk_Rejected']
        assert note.readers == ['TMLRE', 'TMLRE/Paper2/Action_Editors', 'TMLRE/Paper2/Authors']
        assert note.writers == ['TMLRE', 'TMLRE/Paper2/Action_Editors']
        assert note.signatures == ['TMLRE/Paper2/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Ana_Celeste_TMLRE1']
        ...
        assert invitations[0].id == 'TMLRE/Paper2/-/Official_Comment'
        ...
        with pytest.raises(openreview.OpenReviewException, match=r'Can not edit assignments for this submission: TMLRE/Desk_Rejected'):
        ...
        withdraw_note = test_client.post_note_edit(invitation='TMLRE/Paper3/-/Withdrawal',
        ...
        assert note
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Withdrawn']
        assert note.readers == ['TMLRE', 'TMLRE/Paper3/Action_Editors', 'TMLRE/Paper3/Authors']
        assert note.writers == ['TMLRE', 'TMLRE/Paper3/Authors']
        assert note.signatures == ['TMLRE/Paper3/Authors']
        ...
        assert note.content['venueid']['value'] == 'TMLRE/Withdrawn_Submission'
        assert note.content['_bibtex']['value'] == '''@article{
anonymous''' + str(datetime.datetime.fromtimestamp(note.cdate/1000).year) + '''paper,
title={Paper title 3},
author={Anonymous},
...
note={Withdrawn}
        ...
        openreview_client.post_invitation_edit(
            ...
            invitation=openreview.api.Invitation(
                id='TMLRE/-/Preferred_Emails',
                cdate=openreview.tools.datetime_millis(datetime.datetime.now()) + 2000,
            )
        )

        helpers.await_queue_edit(openreview_client, edit_id='TMLRE/-/Preferred_Emails-0-0', count=2)

        ## Check preferred emails
        assert openreview_client.get_edges_count(invitation='TMLRE/-/Preferred_Emails') == 13
        ...
        messages = journal.client.get_messages(
            to='david@mailone.com', subject='[TMLRE] Assignment to review new TMLRE submission Paper title UPDATED')
        assert len(messages) == 0
        ...
        helpers.await_queue_edit(openreview_client, edit_id=paper_assignment_edge.id)
        messages = journal.client.get_messages(to = 'david@mailone.com', subject = '[TMLRE] Assignment to review new TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi David Belanger,

With this email, we request that you submit, within 2 weeks ({(datetime.datetime.now() + datetime.timedelta(weeks = 2)).strftime("%b %d")}) a review for your newly assigned TMLRE submission "1: Paper title UPDATED". If the submission is longer than 12 pages (excluding any appendix), you may request more time to the AE.

Please acknowledge on OpenReview that you have received this review assignment by following this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/Reviewers/-/~David_Belanger1/Assignment/Acknowledgement

As a reminder, reviewers are **expected to accept all assignments** for submissions that fall within their expertise and annual quota (6 papers). Acceptable exceptions are 1) if you have an active, unsubmitted review for another TMLRE submission or 2) situations where exceptional personal circumstances (e.g. vacation, health problems) render you incapable of performing your reviewing duties. Based on the above, if you think you should not review this submission, contact your AE directly (you can do so by leaving a comment on OpenReview, with only the Action Editor as Reader).

To submit your review, please follow this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Review or check your tasks in the Reviewers Console: https://openreview.net/group?id=TMLRE/Reviewers#reviewer-tasks

Once submitted, your review will become privately visible to the authors and AE. Then, as soon as 3 reviews have been submitted, all reviews will become publicly visible. For more details and guidelines on performing your review, visit jmlr.org/tmlre.
...
'''
        ...
        messages = journal.client.get_messages(to = 'david@mailone.com', subject = '[TMLRE] Acknowledgement of Reviewer Responsibility')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi David Belanger,

TMLRE operates somewhat differently to other journals and conferences. As a new reviewer, we'd like you to read and acknowledge some critical points of TMLRE that might differ from your previous reviewing experience.

To perform this quick task, simply visit the following link: https://openreview.net/forum?id={forum_notes[0].id}&invitationId=TMLRE/Reviewers/-/~David_Belanger1/Responsibility/Acknowledgement
...
'''
        ...
        messages = journal.client.get_messages(to = 'carlos@mailthree.com', subject = '[TMLRE] Assignment to review new TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Carlos Mondragon,

With this email, we request that you submit, within 2 weeks ({(datetime.datetime.now() + datetime.timedelta(weeks = 2)).strftime("%b %d")}) a review for your newly assigned TMLRE submission "1: Paper title UPDATED". If the submission is longer than 12 pages (excluding any appendix), you may request more time to the AE.

Please acknowledge on OpenReview that you have received this review assignment by following this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Reviewers/-/Assignment-0-0')

        messages = journal.client.get_messages(subject = '[TMLRE] You are late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 1
...
Number of days late: 1
Link: https://openreview.net/group?id=TMLRE/Action_Editors#action-editor-tasks
...
'''
        ...
        messages = journal.client.get_messages(to = 'javier@mailtwo.com', subject = '[TMLRE] Assignment to review new TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Javier Burroni,

With this email, we request that you submit, within 2 weeks ({(datetime.datetime.now() + datetime.timedelta(weeks = 2)).strftime("%b %d")}) a review for your newly assigned TMLRE submission "1: Paper title UPDATED". If the submission is longer than 12 pages (excluding any appendix), you may request more time to the AE.

Please acknowledge on OpenReview that you have received this review assignment by following this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/Reviewers/-/~Javier_Burroni1/Assignment/Acknowledgement
...
'''
        ...
        raia_client.add_members_to_group('TMLRE/Reviewers', 'antony@irobot.com')
        paper_assignment_edge = joelle_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Assignment',
        ...
        review = openreview_client.get_note(david_review_note['note']['id'])
        assert review.readers == ['TMLRE/Editors_In_Chief', david_anon_groups[0].id]
        ...
        assert len(invitations) == 1
        assert f"{venue_id}/Paper1/-/Review"  in [i.id for i in invitations]
        assert f"{venue_id}/Paper1/-/Official_Comment" not in [i.id for i in invitations]
        ...
        ## Post an official comment from the authors, reply to forum
        comment_note = test_client.post_note_edit(invitation=f'{venue_id}/Paper1/-/Official_Comment',
            ...
            note=Note(
                signatures=[f"{venue_id}/Paper1/Authors"],
                readers=['TMLRE/Editors_In_Chief', 'TMLRE/Paper1/Action_Editors', 'TMLRE/Paper1/Reviewers', 'TMLRE/Paper1/Authors'],
                forum=note_id_1,
                replyto=note_id_1,
                content={
                    'title': { 'value': 'Eagerly waiting' },
                    'comment': { 'value': 'We are eagerly awaiting reviews!' }
                }
            )
        )
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Official Comment posted on submission 1: Paper title UPDATED')
        assert len(messages) == 11

        ## reviewer cannot post a comment yet
        with pytest.raises(openreview.OpenReviewException, match=r'David Belanger is not an invitee of TMLRE/Paper1/-/Official_Comment'):
            comment_note = david_client.post_note_edit(invitation=f'{venue_id}/Paper1/-/Official_Comment',
                signatures=[david_anon_groups[0].id],
                note=Note(
                    signatures=[david_anon_groups[0].id],
                    readers=['TMLRE/Editors_In_Chief', 'TMLRE/Paper1/Action_Editors', david_anon_groups[0].id, 'TMLRE/Paper1/Authors'],
                    forum=note_id_1,
                    replyto=comment_note['note']['id'],
                    content={
                        'title': { 'value': 'Still working on it' },
                        'comment': { 'value': 'I am almost done with my review!' }
                    }
                )
            )
        ...
        with pytest.raises(openreview.OpenReviewException, match=r'Editors In Chief must be readers of the comment'):
            comment_note = joelle_client.post_note_edit(invitation=f'{venue_id}/Paper1/-/Official_Comment',
                ...
                note=Note(
                    signatures=[joelle_paper1_anon_group.id],
                    readers=['TMLRE/Paper1/Action_Editors', 'TMLRE/Paper1/Reviewers'],
                    forum=note_id_1,
                    replyto=note_id_1,
                    content={
                        'title': { 'value': 'Reminder' },
                        'comment': { 'value': 'Please complete your reviews by tomorrow' }
                    }
                )
            )
        ...
        comment_note = raia_client.post_note_edit(invitation=f'{venue_id}/Paper1/-/Official_Comment',
            ...
            note=Note(
                signatures=['TMLRE/Editors_In_Chief'],
                readers=['TMLRE/Editors_In_Chief'],
                forum=note_id_1,
                replyto=note_id_1,
                content={
        ...
        messages = journal.client.get_messages(to='raia@mail.com', subject = '[TMLRE] Official Comment posted on submission 1: Paper title UPDATED')
        assert len(messages) == 1
        ...
        assert note
        assert note.invitations == ['TMLRE/Paper1/-/Public_Comment']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper1/Action_Editors', '~Peter_Snow1']
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Public Comment posted on submission 1: Paper title UPDATED')
        assert len(messages) == 9
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Public Comment posted on submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['to'] == 'joelle@mailseven.com'
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,
...
'''
        ...
        assert note
        assert note.invitations == ['TMLRE/Paper1/-/Public_Comment', 'TMLRE/Paper1/-/Moderation']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper1/Action_Editors']
        assert note.signatures == ['~Peter_Snow1']
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Review-0-0')

        messages = journal.client.get_messages(subject = '[TMLRE] You are late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 3
        messages = journal.client.get_messages(to = 'carlos@mailthree.com', subject = '[TMLRE] You are late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 1
...
'''
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Reviewer is late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 0
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Review-0-1')
...
''' for message in messages)
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Review-0-2')
...
''' for message in messages)
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Review-1-0')
        
        messages = journal.client.get_messages(subject = '[TMLRE] Fewer than 3 ACKs for the paper 1: Paper title UPDATED')
        assert len(messages) == 2
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Review-0-3')
        ...
        messages = journal.client.get_messages(to= 'raia@mail.com', subject = '[TMLRE] Reviewer is late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 2
...
''' for message in messages)
        ...
        messages = journal.client.get_messages(to= 'joelle@mailseven.com', subject = '[TMLRE] Reviewer is late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 6
        assert any(message['content']['text'] == f'''Hi Joelle Pineau,
...
''' for message in messages)
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement-0-0')

        messages = journal.client.get_messages(to = 'carlos@mailthree.com', subject = '[TMLRE] You are late in performing a task for assigned paper 1: Paper title UPDATED')
        assert len(messages) == 5
        assert any(message['content']['text'] == f'''Hi Carlos Mondragon,

Our records show that you have not acknowledged your TMLRE assignment yet:
...
''' for message in messages)
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement-0-1')
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement-0-2')
...
'''
        ...
        late_reviewers = journal.get_late_invitees('TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement')
        ...
        assignment_ack_note = carlos_client.post_note_edit(invitation=f'TMLRE/Paper1/Reviewers/-/~Carlos_Mondragon1/Assignment/Acknowledgement',
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Assignment Acknowledgement posted on submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,
...
'''
        ...
        messages = openreview_client.get_messages(to = 'carlos@mailthree.com', subject = '[TMLRE] Start of author discussion for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Carlos Mondragon,
...
'''
        ...
        messages = openreview_client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Start of author discussion for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,
...
'''
        ...
        messages = openreview_client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Too many reviewers assigned to TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,
...
'''
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Submit official recommendation for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 5
        messages = journal.client.get_messages(to= 'hugo@mailsix.com', subject = '[TMLRE] Submit official recommendation for TMLRE submission 1: Paper title UPDATED')
        assert messages[0]['content']['text'] == f'''Hi Hugo Larochelle,

Thank you for submitting your review and engaging with the authors of TMLRE submission "1: Paper title UPDATED".
...
We ask that you submit your recommendation within 2 weeks ({(datetime.datetime.now() + datetime.timedelta(weeks = 4)).strftime("%b %d")}). To do so, please follow this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Official_Recommendation

For more details and guidelines on performing your review, visit jmlr.org/tmlre.
...
Please note that responding to this email will direct your reply to joelle@mailseven.com.
'''
        messages = journal.client.get_messages(subject = '[TMLRE] Reviewers must submit official recommendation for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1

        messages = openreview_client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Discussion period ended for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 0
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Evaluate reviewers and submit decision for TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,

Thank you for overseeing the review process for TMLRE submission "1: Paper title UPDATED".
...
We ask that you submit your decision **within 1 week** ({(datetime.datetime.now() + datetime.timedelta(weeks = 1)).strftime("%b %d")}). To do so, please follow this link: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Decision
...
'''
        ...
        assert 'TMLRE/Decision_Pending' == submission.content['venueid']['value']
        ...
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper1/-/Decision_Approval',
                            signatures=['TMLRE/Editors_In_Chief'],
                            note=Note(
                                content= {
                                    'approval': { 'value': 'I approve the AE\'s decision.' },
        ...
        assert decision_note.writers == ['TMLRE']
        assert decision_note.nonreaders == []

        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Decision approved for submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,
...
'''        
        ...
        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Decision for your TMLRE submission 1: Paper title UPDATED')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,

We are happy to inform you that, based on the evaluation of the reviewers and the recommendation of the assigned Action Editor, your TMLRE submission "1: Paper title UPDATED" is accepted as is.
...
We thank you for your contribution to TMLRE and congratulate you for your successful submission!
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Camera_Ready_Revision-0-0')        
        
        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] You are late in performing a task for your paper 1: Paper title UPDATED')
        assert len(messages) == 1
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Camera_Ready_Revision-0-1')        
        ...
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/Paper1/-/Camera_Ready_Revision']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper1/Authors']
        assert note.signatures == ['TMLRE/Paper1/Authors']
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Review camera ready version for TMLRE paper 1: Paper title VERSION 2')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,

The authors of TMLRE paper 1: Paper title VERSION 2 have now submitted the deanonymized camera ready version of their work.

As your final task for this submission, please verify that the camera ready manuscript complies with the TMLRE stylefile, with all author information inserted in the manuscript as well as the link to the OpenReview page for the submission.

Moreover, if the paper was accepted with minor revision, verify that the changes requested have been followed.

Visit the following link to perform this task: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Camera_Ready_Verification
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Camera_Ready_Verification-0-1')

        messages = journal.client.get_messages(subject = '[TMLRE] You are late in performing a task for assigned paper 1: Paper title VERSION 2')
        assert len(messages) == 1

        messages = journal.client.get_messages(subject = '[TMLRE] AE is late in performing a task for assigned paper 1: Paper title VERSION 2')
        assert len(messages) == 2

        messages = journal.client.get_messages(to='raia@mail.com', subject = '[TMLRE] AE is late in performing a task for assigned paper 1: Paper title VERSION 2')
        assert len(messages) == 1
...
'''
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper1/-/Camera_Ready_Verification-0-2')
        ...
        verification_note = joelle_client.post_note_edit(invitation='TMLRE/Paper1/-/Camera_Ready_Verification',
                            signatures=[joelle_paper1_anon_group.id],
                            note=Note(
                                signatures=[joelle_paper1_anon_group.id],
                                content= {
                                    'verification': { 'value': 'I confirm that camera ready manuscript complies with the TMLRE stylefile and, if appropriate, includes the minor revisions that were requested.' }
                                 }
                            ))
        ...
        assert journal.get_late_invitees('TMLRE/Paper1/-/Camera_Ready_Verification') == []

        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Camera ready version accepted for your TMLRE submission 1: Paper title VERSION 2')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,
...
We thank you again for your contribution to TMLRE and congratulate you for your successful submission!
...
'''
        ...
        assert note.replyto is None
        assert note.pdate
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/Paper1/-/Camera_Ready_Revision', 'TMLRE/-/Accepted']
        assert note.readers == ['everyone']
...
note={Featured Certification, Reproducibility Certification, Expert Certification}
        ...
        helpers.await_queue_edit(openreview_client, invitation='TMLRE/-/Accepted')
        ...
        assert note.forum == note_id_1
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/Paper1/-/Camera_Ready_Revision', 'TMLRE/-/Accepted', 'TMLRE/Paper1/-/EIC_Revision']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE']
        assert note.signatures == ['TMLRE/Paper1/Authors']
...
note={Featured Certification, Reproducibility Certification, Expert Certification, Outstanding Certification}
        ...
        ## Retract the paper
        retraction_note = test_client.post_note_edit(invitation='TMLRE/Paper1/-/Retraction',
                            signatures=[f"{venue_id}/Paper1/Authors"],
                            note=Note(
                                signatures=[f"{venue_id}/Paper1/Authors"],
                                content= {
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Authors request to retract TMLRE submission 1: Paper title VERSION 2')
        assert len(messages) == 2
        messages = journal.client.get_messages(to='raia@mail.com', subject = '[TMLRE] Authors request to retract TMLRE submission 1: Paper title VERSION 2')
        assert messages[0]['content']['text'] == f'''Hi Raia Hadsell,

The authors of paper 1: Paper title VERSION 2 are requesting to retract the paper. An EIC must confirm and accept the retraction: https://openreview.net/forum?id={note_id_1}&invitationId=TMLRE/Paper1/-/Retraction_Approval
...
'''
        ...
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper1/-/Retraction_Approval',
                            signatures=[f"{venue_id}/Editors_In_Chief"],
                            note=Note(
                                signatures=[f"{venue_id}/Editors_In_Chief"],
                                content= {
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Decision available for retraction request of TMLRE submission 1: Paper title VERSION 2')
        assert len(messages) == 4
        messages = journal.client.get_messages(to='test@mail.com', subject = '[TMLRE] Decision available for retraction request of TMLRE submission 1: Paper title VERSION 2')
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,

As TMLRE Editors-in-Chief, we have submitted our decision on your request to retract your accepted paper at TMLRE "1: Paper title VERSION 2".
...
'''
        ...
        assert note.forum == note_id_1
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/Paper1/-/Revision', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/Paper1/-/Camera_Ready_Revision', 'TMLRE/-/Accepted', 'TMLRE/Paper1/-/EIC_Revision', 'TMLRE/-/Retracted']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE']
        assert note.signatures == ['TMLRE/Paper1/Authors']
        assert note.content['authorids']['value'] == ['~Melisa_TMLRE1', '~SomeFirstName_User1', '~Ana_Celeste_TMLRE1', '~Andrew_McCallumm1']
        ...
        assert note.content['venueid']['value'] == 'TMLRE/Retracted_Acceptance'
        assert note.content['title']['value'] == 'Paper title VERSION 2'
        assert note.content['abstract']['value'] == 'Paper abstract'
        assert note.content['_bibtex']['value'] == '''@article{
tmlre''' + str(datetime.datetime.fromtimestamp(note.cdate/1000).year) + '''paper,
...
note={Retracted after acceptance}
    ...
    def test_rejected_submission(self, journal, openreview_client, test_client, helpers):
        ...
        submission_note_4 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper4/Action_Editors/-/Recommendation')        
        ...
        under_review_note = raia_client.post_note_edit(invitation= 'TMLRE/Paper4/-/Review_Approval',
                                    signatures=[f'{venue_id}/Editors_In_Chief'],
                                    note=Note(content={
                                        'under_review': { 'value': 'Appropriate for Review' }
                                    }))
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_4, invitation='TMLRE/-/Under_Review')
        ...
        messages = journal.client.get_messages(to = 'david@mailone.com', subject = '[TMLRE] Assignment to review new TMLRE submission 4: Paper title 4')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi David Belanger,

With this email, we request that you submit, within 4 weeks ({(datetime.datetime.now() + datetime.timedelta(weeks = 4)).strftime("%b %d")}) a review for your newly assigned TMLRE submission "4: Paper title 4". If the submission is longer than 12 pages (excluding any appendix), you may request more time to the AE.

Please acknowledge on OpenReview that you have received this review assignment by following this link: https://openreview.net/forum?id={note_id_4}&invitationId=TMLRE/Paper4/Reviewers/-/~David_Belanger1/Assignment/Acknowledgement

As a reminder, reviewers are **expected to accept all assignments** for submissions that fall within their expertise and annual quota (6 papers). Acceptable exceptions are 1) if you have an active, unsubmitted review for another TMLRE submission or 2) situations where exceptional personal circumstances (e.g. vacation, health problems) render you incapable of performing your reviewing duties. Based on the above, if you think you should not review this submission, contact your AE directly (you can do so by leaving a comment on OpenReview, with only the Action Editor as Reader).

To submit your review, please follow this link: https://openreview.net/forum?id={note_id_4}&invitationId=TMLRE/Paper4/-/Review or check your tasks in the Reviewers Console: https://openreview.net/group?id=TMLRE/Reviewers#reviewer-tasks
...
'''
        ...
        edges = joelle_client.get_grouped_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', groupby='weight')
        assert len(edges) == 2
        ...
        assert openreview_client.get_invitation(f'{venue_id}/Paper4/-/~Ana_Celeste_TMLRE1_Volunteer_to_Review_Approval')
        ...
        invitation = openreview_client.get_invitation(f'{venue_id}/Paper4/-/~Ana_Celeste_TMLRE1_Volunteer_to_Review_Approval') 
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Request to review TMLRE submission "4: Paper title 4" has been submitted')
        assert len(messages) == 2
        assert messages[-1]['content']['text'] == f'''Hi Joelle Pineau,

This is to inform you that an OpenReview user (Tom Rain) has requested to review TMLRE submission 4: Paper title 4, which you are the AE for.
...
We thank you for your contribution to TMLRE!
...
'''
        ...
        assert comment.readers == [ 'TMLRE/Editors_In_Chief', 'TMLRE/Paper4/Action_Editors', '~Peter_Snow1' ]

        messages = journal.client.get_messages(to = 'petersnow@yahoo.com', subject = '[TMLRE] Comment posted on submission 4: "Paper title 4" pertaining to volunteer reviewing')
        assert len(messages) == 1
...
To view the comment, click here: https://openreview.net/forum?id={note_id_4}&noteId={comment.id}'''
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Comment posted on submission 4: "Paper title 4" pertaining to volunteer reviewing')
        assert len(messages) == 1
...
To view the comment, click here: https://openreview.net/forum?id={note_id_4}&noteId={comment.id}'''
        ...
        paper_assignment_edges = openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', tail='~Peter_Snow1', head=note_id_4)
        ...
        messages = journal.client.get_messages(to = 'petersnow@yahoo.com', subject = '[TMLRE] Request to review TMLRE submission "4: Paper title 4" has been accepted')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Peter Snow,

This is to inform you that your request to act as a reviewer for TMLRE submission 4: Paper title 4 has been accepted by the Action Editor (AE).
...
To submit your review, please follow this link: https://openreview.net/forum?id={note_id_4}&invitationId=TMLRE/Paper4/-/Review or check your tasks in the Reviewers Console: https://openreview.net/group?id=TMLRE/Reviewers
...
'''
        ...
        messages = journal.client.get_messages(to = 'petersnow@yahoo.com', subject = '[TMLRE] Assignment to review new TMLRE submission 4: Paper title 4')
        assert len(messages) == 0

        assert not journal.client.get_messages(to = 'petersnow@yahoo.com', subject = '[TMLRE] Acknowledgement of Reviewer Responsibility')
        assert not openreview.tools.get_invitation(openreview_client, 'TMLRE/Reviewers/-/~Peter_Snow1/Responsibility/Acknowledgement')
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Review posted on TMLRE submission 4: Paper title 4')
        ...
        ## Check pending review edges
        edges = joelle_client.get_edges_count(invitation='TMLRE/Reviewers/-/Pending_Reviews')
        assert edges == 5
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Carlos_Mondragon1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Javier_Burroni1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~David_Belanger1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Hugo_Larochelle1')[0].weight == 1
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Antony_Bal1')[0].weight == 0
        ...
        decision_note = joelle_client.get_note(decision_note['note']['id'])
        assert decision_note.readers == ['TMLRE/Editors_In_Chief', f"{venue_id}/Paper4/Action_Editors"]

        ## EIC approves the decision
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper4/-/Decision_Approval',
                            signatures=['TMLRE/Editors_In_Chief'],
                            note=Note(
                            content= {
                                'approval': { 'value': 'I approve the AE\'s decision.' },
        ...
        helpers.await_queue_edit(openreview_client, invitation='TMLRE/-/Rejected')

        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Decision for your TMLRE submission 4: Paper title 4')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,

We are sorry to inform you that, based on the evaluation of the reviewers and the recommendation of the assigned Action Editor, your TMLRE submission "4: Paper title 4" is rejected.
...
In any case, your submission will remain publicly available on OpenReview. You may decide to reveal your identity and deanonymize your submission on the OpenReview page. Doing so will however preclude you from submitting any revised version of the manuscript to TMLRE.
...
'''
        ...
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/-/Rejected']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper4/Authors']
        assert note.signatures == ['TMLRE/Paper4/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
        assert note.content['venue']['value'] == 'Rejected by TMLRE'
        assert note.content['venueid']['value'] == 'TMLRE/Rejected'
...
note={Rejected}
        ...
        deanonymize_authors_note = test_client.post_note_edit(invitation='TMLRE/Paper4/-/Authors_De-Anonymization',
                            signatures=['TMLRE/Paper4/Authors'],
                            note=Note(
                            content= {
        ...
        assert note.forum == note_id_4
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/-/Rejected', 'TMLRE/-/Authors_Release']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper4/Authors']
        assert note.signatures == ['TMLRE/Paper4/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
...
note={Rejected}
        ...
        ## Check pending review edges
        edges_count = joelle_client.get_edges_count(invitation='TMLRE/Reviewers/-/Pending_Reviews')
        assert edges_count == 5
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~David_Belanger1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Carlos_Mondragon1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Javier_Burroni1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Antony_Bal1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Hugo_Larochelle1')[0].weight == 0
    ...
    def test_eic_submission(self, journal, openreview_client, test_client, helpers):
        ...
        submission_note_5 = raia_client.post_note_edit(invitation='TMLRE/-/Submission',
        ...
        openreview_client.get_invitation('TMLRE/Paper5/Action_Editors/-/Recommendation')        

        # Assign Action Editor
        paper_assignment_edge = cho_client.post_edge(openreview.api.Edge(invitation='TMLRE/Action_Editors/-/Assignment',
            readers=[venue_id, editor_in_chief_group_id, '~Joelle_Pineau1'],
            writers=[venue_id, editor_in_chief_group_id],
            signatures=[editor_in_chief_group_id],
            head=note_id_5,
        ...
        messages = journal.client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Attention: you\'ve been assigned a submission authored by an EIC')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Joelle Pineau,

You have just been assigned a submission that is authored by one (or more) TMLRE Editors-in-Chief. OpenReview is set up such that the EIC in question will not have access through OpenReview to the identity of the reviewers you'll be assigning. 

However, be mindful not to discuss the submission by email through TMLRE's EIC mailing lists (tmlre@jmlr.org or TMLRE-editors@jmlr.org), since all EICs receive these emails. Instead, if you need to reach out to EICs by email, only contact the non-conflicted EICs, directly.
...
'''        
        ...
        helpers.await_queue_edit(openreview_client, 'TMLRE/Paper5/-/Review_Approval-0-2')

        messages = journal.client.get_messages(to= 'raia@mail.com', subject = '[TMLRE] AE is late in performing a task for assigned paper 5: Paper title 5')
        assert len(messages) == 0
        ...
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper5/-/Review_Approval',
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_5, invitation='TMLRE/-/Under_Review')
        ...
        ## Assign Javier Burroni
        ## try editing the assignmente edge being the author and get an error
        paper_assignment_edge = openreview.api.Edge(invitation='TMLRE/Reviewers/-/Assignment',
        ...
        decision_note = joelle_client.get_note(decision_note['note']['id'])
        assert decision_note.readers == ['TMLRE/Editors_In_Chief', f"{venue_id}/Paper5/Action_Editors"]

        ## EIC approves the decision
        with pytest.raises(openreview.OpenReviewException, match=r'NotInviteeError'):
            approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper5/-/Decision_Approval',
                                signatures=['TMLRE/Editors_In_Chief'],
                                note=Note(
                                content= {
                                    'approval': { 'value': 'I approve the AE\'s decision.' },
        ...
        approval_note = cho_client.post_note_edit(invitation='TMLRE/Paper5/-/Decision_Approval',
                            signatures=['TMLRE/Editors_In_Chief'],
                            note=Note(
                            content= {
                                'approval': { 'value': 'I approve the AE\'s decision.' },
        ...
        messages = journal.client.get_messages(to = 'raia@mail.com', subject = '[TMLRE] Decision for your TMLRE submission 5: Paper title 5')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Raia Hadsell,

We are happy to inform you that, based on the evaluation of the reviewers and the recommendation of the assigned Action Editor, your TMLRE submission "5: Paper title 5" is accepted with minor revision.
...
'''
        ...
        withdraw_note = raia_client.post_note_edit(invitation='TMLRE/Paper5/-/Withdrawal',
    ...
    def test_withdraw_submission(self, journal, openreview_client, helpers):
        ...
        submission_note_6 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper6/Action_Editors/-/Recommendation')        
        ...
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper6/-/Review_Approval',
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_6, invitation='TMLRE/-/Under_Review')
        ...
        withdraw_note = test_client.post_note_edit(invitation='TMLRE/Paper6/-/Withdrawal',
        ...
        assert note
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/-/Withdrawn']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper6/Authors']
        assert note.signatures == ['TMLRE/Paper6/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
        assert note.content['venue']['value'] == 'Withdrawn by Authors'
        assert note.content['venueid']['value'] == 'TMLRE/Withdrawn_Submission'

        edits = openreview_client.get_note_edits(note_id=note_id_6, invitation='TMLRE/-/Withdrawn')
        assert len(edits) == 1
        helpers.await_queue_edit(openreview_client, edit_id=edits[0].id)
        ...
        messages = journal.client.get_messages(subject = '[TMLRE] Authors have withdrawn TMLRE submission 6: Paper title 6')
        assert len(messages) == 4

        deanonymize_authors_note = test_client.post_note_edit(invitation='TMLRE/Paper6/-/Authors_De-Anonymization',
                            signatures=['TMLRE/Paper6/Authors'],
                            note=Note(
                            content= {
                                'confirmation': { 'value': 'I want to reveal all author names on behalf of myself and my co-authors.' }
                            }))
        ...
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review', 'TMLRE/-/Withdrawn', 'TMLRE/-/Authors_Release']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper6/Authors']
        assert note.signatures == ['TMLRE/Paper6/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
...
note={Withdrawn}
    ...
    def test_submitted_submission(self, journal, openreview_client, helpers):
        ...
        submission_note_7 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper7/Action_Editors/-/Recommendation')        
        ...
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper7/-/Review_Approval',
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_7, invitation='TMLRE/-/Under_Review')
        ...
        helpers.await_queue_edit(openreview_client, edit_id=Volunteer_to_Review_note['id'])
        assert 'TMLRE/Editors_In_Chief' in Volunteer_to_Review_note['note']['readers']
        ...
        assert 'TMLRE/Editors_In_Chief' in Volunteer_to_Review_approval_note['note']['readers']

        helpers.await_queue_edit(openreview_client, edit_id=Volunteer_to_Review_approval_note['id'])

        messages = journal.client.get_messages(to = 'tom@mail.com', subject = '[TMLRE] Request to review TMLRE submission "7: Paper title 7" was not accepted')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Tom Rain,

This is to inform you that your request to act as a reviewer for TMLRE submission 7: Paper title 7 was not accepted by the Action Editor (AE). If you would like to know more about the reason behind this decision, you can click here: https://openreview.net/forum?id={note_id_7}&noteId={Volunteer_to_Review_approval_note['note']['id']}.
...
'''
        ...
        submission_note_8 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper8/Action_Editors/-/Recommendation')        
        ...
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper8/-/Review_Approval',
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_8, invitation='TMLRE/-/Under_Review')
        ...
        helpers.await_queue_edit(openreview_client, edit_id=volunteer_to_review_approval_note['id'])
        assignment_edge = openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_8, tail='~David_Belanger1')[0]
        ...
        helpers.await_queue_edit(openreview_client, edit_id=Volunteer_to_Review_approval_note['id'])
        assignment_edge = openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_8, tail='~Peter_Snow1')[0]
        helpers.await_queue_edit(openreview_client, edit_id=assignment_edge.id)
        ...
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Carlos_Mondragon1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Javier_Burroni1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~David_Belanger1')[0].weight == 2
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Hugo_Larochelle1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Antony_Bal1')[0].weight == 0
        ...
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Carlos_Mondragon1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Javier_Burroni1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~David_Belanger1')[0].weight == 1
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Hugo_Larochelle1')[0].weight == 0
        assert joelle_client.get_edges(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~Antony_Bal1')[0].weight == 0
        ...
        current_assignment = raia_client.get_edges(invitation='TMLRE/Action_Editors/-/Assignment', head=note_id_8)[0]
    ...
    def test_desk_rejected_submission_by_eic(self, journal, openreview_client, helpers):
        ...
        submission_note_9 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper9/Action_Editors/-/Recommendation')        

        ## Desk Reject by EIC
        desk_reject_note = raia_client.post_note_edit(invitation= 'TMLRE/Paper9/-/Desk_Rejection',
                                    signatures=[f'{venue_id}/Editors_In_Chief'],
                                    note=Note(content={
        ...
        assert note.replyto is None
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Desk_Rejected']
        assert note.readers == ['TMLRE', 'TMLRE/Paper9/Action_Editors', 'TMLRE/Paper9/Authors']
        assert note.writers == ['TMLRE', 'TMLRE/Paper9/Action_Editors']
        assert note.signatures == ['TMLRE/Paper9/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
        assert note.content['venue']['value'] == 'Desk rejected by TMLRE'
        assert note.content['venueid']['value'] == 'TMLRE/Desk_Rejected'
        ...
        messages = journal.client.get_messages(to = 'test@mail.com', subject = '[TMLRE] Decision for your TMLRE submission 9: Paper title 9')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi SomeFirstName User,

We are sorry to inform you that, after consideration by the Editors-in-Chief, your TMLRE submission "9: Paper title 9" has been rejected without further review.
...
'''
    ...
    def test_resubmission(self, journal, openreview_client, helpers):
        ...
        rejected_submission = openreview_client.get_notes(invitation='TMLRE/-/Submission', number=4)[0]
        ## Resubmit rejected paper
        submission_note_10 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
                    'title': { 'value': 'Paper title 4' },
                    ...
                    'human_subjects_reporting': { 'value': 'Not applicable'},
                    'submission_length': { 'value': 'Long submission (more than 12 pages of main content)'},
                    'previous_TMLRE_submission_url': { 'value': f'https://openreview.net/forum?id={rejected_submission.id}' }
                }
            ))
        ...
        openreview_client.get_invitation('TMLRE/Paper10/Action_Editors/-/Recommendation')        
        ...
        ## Accept the submission 8
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper10/-/Review_Approval',
        ...
        edits = openreview_client.get_note_edits(note_id=note_id_10, invitation='TMLRE/-/Under_Review')
    ...
    def test_submission_with_many_authors(self, journal, openreview_client, test_client, helpers):
        ...
        for i in alc:
            for j in alc[:5]:
                helpers.create_user(f'author_{i}{j}@mail.com', 'Author', f'TMLRE {i}{j}')
                authors.append(f'Author TMLRE {i}{j}')
                authorids.append(f'~Author_TMLRE_{i}{j}1')
        
        submission_note_11 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
                    'title': { 'value': 'Paper title 4' },
        ...
        openreview_client.get_invitation('TMLRE/Paper11/Action_Editors/-/Recommendation')        
        ...
        ## Accept the submission 8
        under_review_note = joelle_client.post_note_edit(invitation= f'TMLRE/Paper{note.number}/-/Review_Approval',
        ...
        assert len(edits) == 3
        assert edits[0].invitation == 'TMLRE/-/Under_Review'
        helpers.await_queue_edit(openreview_client, edit_id=edits[0].id)
              
        journal.invitation_builder.expire_paper_invitations(note)

        invitation = openreview_client.get_invitation('TMLRE/Paper11/Reviewers/-/Assignment')
        assert invitation.expdate is not None
    ...
    def test_decline_desk_rejection(self, journal, openreview_client, helpers):
        ...
        submission_note_12 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        openreview_client.get_invitation('TMLRE/Paper12/Action_Editors/-/Recommendation')        
        ...
        desk_reject_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper12/-/Review_Approval',
        ...
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper12/-/Desk_Rejection_Approval',
                            signatures=[f"{venue_id}/Editors_In_Chief"],
                            note=Note(
                                signatures=[f"{venue_id}/Editors_In_Chief"],
                                forum=note_id_12,
        ...
        notes = openreview_client.get_notes(invitation='TMLRE/Paper12/-/Review_Approval', signature='TMLRE/Editors_In_Chief')
        edits = openreview_client.get_note_edits(notes[0].id)
        edit = [e for e in edits if 'TMLRE/Paper12/-/Review_Approval' == e.invitation][0]
        helpers.await_queue_edit(openreview_client, edit_id=edit.id)

        note = joelle_client.get_note(note_id_12)
        assert note
        assert note.odate
        assert note.invitations == ['TMLRE/-/Submission', 'TMLRE/-/Edit', 'TMLRE/-/Under_Review']
        assert note.readers == ['everyone']
        assert note.writers == ['TMLRE', 'TMLRE/Paper12/Authors']
        assert note.signatures == ['TMLRE/Paper12/Authors']
        assert note.content['authorids']['value'] == ['~SomeFirstName_User1', '~Melisa_TMLRE1']
        assert note.content['venue']['value'] == 'Under review for TMLRE'
        assert note.content['venueid']['value'] == 'TMLRE/Under_Review'
        assert note.content['assigned_action_editor']['value'] == '~Joelle_Pineau1'
...
note={Under review}
        ...
        messages = openreview_client.get_messages(to = 'joelle@mailseven.com', subject = '[TMLRE] Perform reviewer assignments for TMLRE submission 12: Paper title 12')
        assert len(messages) == 1

        messages = openreview_client.get_messages(subject='[TMLRE] Review Approval edited on submission 12: Paper title 12')
    ...
    def test_archived_action_editor(self, journal, openreview_client, test_client, helpers):
        ...
        submission_note_13 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        assert openreview_client.get_invitation('TMLRE/Paper13/Action_Editors/-/Recommendation')
        ...
        ## Archive Joelle
        raia_client.remove_members_from_group(raia_client.get_group('TMLRE/Action_Editors'), '~Joelle_Pineau1')
        raia_client.add_members_to_group(raia_client.get_group('TMLRE/Action_Editors/Archived'), '~Joelle_Pineau1')
        ...
        comment_note = joelle_client.post_note_edit(invitation=f'{venue_id}/Paper13/-/Official_Comment',
            ...
            note=Note(
                signatures=[joelle_paper13_anon_group.id],
                readers=['TMLRE/Editors_In_Chief', 'TMLRE/Paper13/Action_Editors'],
                forum=note_id_13,
                replyto=note_id_13,
                content={
                    'comment': { 'value': 'I\'m not sure if I should accept this paper to be under review' }
        ...
        ## Accept the submission 1
        under_review_note = joelle_client.post_note_edit(invitation= 'TMLRE/Paper13/-/Review_Approval',
                                    signatures=[joelle_paper13_anon_group.id],
                                    note=Note(content={
                                        'under_review': { 'value': 'Appropriate for Review' }
                                    }))
        ...
        edits = openreview_client.get_note_edits(note_id_13, invitation='TMLRE/-/Under_Review')
        helpers.await_queue_edit(openreview_client, edit_id=edits[0].id)
        ...
        ## Archive David
        david_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Assignment_Availability',
            signatures=['~David_Belanger1'],
            head='TMLRE/Reviewers',
            tail='~David_Belanger1',
            label='Unavailable'
        ))        
        #raia_client.remove_members_from_group(raia_client.get_group('TMLRE/Reviewers'), '~David_Belanger1')
        #raia_client.add_members_to_group(raia_client.get_group('TMLRE/Reviewers/Archived'), '~David_Belanger1')

        edit_group = raia_client.post_group_edit(
            invitation='TMLRE/Reviewers/Archived/-/Member',
            signatures=['TMLRE'],
            group=openreview.api.Group(
                members={
        ...
        assert '~David_Belanger1' in openreview_client.get_group('TMLRE/Reviewers/Archived').members
        assert '~David_Belanger1' not in openreview_client.get_group('TMLRE/Reviewers').members
        assert openreview_client.get_edges_count(invitation='TMLRE/Reviewers/-/Assignment_Availability', tail='~David_Belanger1') == 0
        assert openreview_client.get_edges_count(invitation='TMLRE/Reviewers/-/Pending_Reviews', tail='~David_Belanger1') == 0
        assert openreview_client.get_edges_count(invitation='TMLRE/Reviewers/-/Custom_Max_Papers', tail='~David_Belanger1') == 0

        # post a reviewer report about an archived reviewer TMLRE/Reviewers/-/Reviewer_Report
        ...
        approval_note = raia_client.post_note_edit(invitation='TMLRE/Paper13/-/Decision_Approval',
            signatures=['TMLRE/Editors_In_Chief'],
            note=Note(
            content= {
                'approval': { 'value': 'I approve the AE\'s decision.' },
        ...
        verification_note = joelle_client.post_note_edit(invitation='TMLRE/Paper13/-/Camera_Ready_Verification',
                            signatures=[joelle_paper13_anon_group.id],
                            note=Note(
                                signatures=[joelle_paper13_anon_group.id],
                                content= {
...
note={}
        ...
        helpers.await_queue_edit(openreview_client, invitation='TMLRE/-/Accepted', count=2)
        ...
        edit_group = raia_client.post_group_edit(
            invitation='TMLRE/Expert_Reviewers/-/Member',
            signatures=['TMLRE'],
            group=openreview.api.Group(
                members={
        ...
        assert openreview_client.get_group('TMLRE/Expert_Reviewers').members == ['~Andrew_McCallumm1', '~Hugo_Larochelle1']
...
note={Expert Certification}
        ...
        raia_client.post_note_edit(invitation='TMLRE/-/Event_Certification',
            signatures=["TMLRE"],
            note=Note(
                id = note_id_13,
                content= {
    ...
    def test_invite_external_reviewers(self, journal, openreview_client, test_client, helpers, selenium, request_page):
        ...
        submission_note_14 = test_client.post_note_edit(invitation='TMLRE/-/Submission',
            signatures=['~SomeFirstName_User1'],
            note=Note(
                content={
        ...
        assert openreview_client.get_invitation(f'TMLRE/Paper{submission.number}/Action_Editors/-/Recommendation')
        ...
        ## Accept the submission 1
        under_review_note = samy_client.post_note_edit(invitation= f'TMLRE/Paper{submission.number}/-/Review_Approval',
                                    signatures=[joelle_paper13_anon_group.id],
                                    note=Note(content={
                                        'under_review': { 'value': 'Appropriate for Review' }
                                    }))
        ...
        edits = openreview_client.get_note_edits(note_id_14, invitation='TMLRE/-/Under_Review')
        helpers.await_queue_edit(openreview_client, edit_id=edits[0].id)

        helpers.await_queue_edit(openreview_client, invitation='TMLRE/-/Under_Review')

        ## Invite external reviewer with profile
        paper_assignment_edge = samy_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Invite_Assignment',
            signatures=[joelle_paper13_anon_group.id],
            head=note_id_14,
        ...
        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='~Rachel_TMLRE1')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Invitation Sent'         

        messages = openreview_client.get_messages(to = 'melisa@mailten.com', subject = '[TMLRE] Invitation to review paper titled "Paper title 14"')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Melisa Bok,\n\nYou were invited to review the paper number: {submission.number}, title: \"Paper title 14\".\n\nAbstract: Paper abstract\n\nPlease respond the invitation clicking the following link:\n\nhttps://openreview.net/invitation?id=TMLRE/Reviewers/-/Assignment_Recruitment&user=~Rachel_TMLRE1&key=da9c8e30376e445b0d18011af6a8ca7863e5037e3f9aebc6aa4294e5aaa5c216&submission_id={submission.id}&inviter=~Samy_Bengio1\n\nThanks,\n\nTMLRE Paper{submission.number} Action Editor {joelle_paper13_anon_group.id.split('_')[-1]}\nSamy Bengio\n\nPlease note that responding to this email will direct your reply to samy@bengio.com.\n'''

        messages = openreview_client.get_messages(to = 'samy@bengio.com', subject = '[TMLRE] Invitation to review paper titled "Paper title 14"')
        assert len(messages) == 1
        ...
        helpers.await_queue_edit(openreview_client, invitation='TMLRE/Reviewers/-/Assignment_Recruitment', count=1)

        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='~Rachel_TMLRE1')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Accepted'         

        assignment_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_14, tail='~Rachel_TMLRE1')
        assert len(assignment_edges) == 1

        helpers.await_queue_edit(openreview_client, edit_id=assignment_edges[0].id)

        messages = openreview_client.get_messages(to = 'melisa@mailten.com', subject = f'[TMLRE] Reviewers Invitation accepted for paper {submission.number}: Paper title 14')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Melisa Bok,\nThank you for accepting the invitation to review the paper number: {submission.number}, title: Paper title 14.\n\nPlease go to the Tasks page and check your TMLRE pending tasks: https://openreview.net/tasks\n\nIf you would like to change your decision, please follow the link in the previous invitation email and click on the \"Decline\" button.\n\nOpenReview Team\n\nPlease note that responding to this email will direct your reply to tmlre@jmlr.org.\n'''

        messages = openreview_client.get_messages(to = 'samy@bengio.com', subject = f'[TMLRE] Reviewers Melisa Bok accepted to review paper {submission.number}: Paper title 14')
        assert len(messages) == 1
        ...
        messages = openreview_client.get_messages(to = 'melisa@mailten.com', subject = f'[TMLRE] Assignment to review new TMLRE submission {submission.number}: Paper title 14')
        assert len(messages) == 0

        messages = openreview_client.get_messages(to = 'melisa@mailten.com', subject = '[TMLRE] Acknowledgement of Reviewer Responsibility')
        assert len(messages) == 0
        ...
        messages = openreview_client.get_messages(to = 'harold@hotmail.com', subject = '[TMLRE] Invitation to review paper titled "Paper title 14"')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi harold@hotmail.com,\n\nYou were invited to review the paper number: {submission.number}, title: \"Paper title 14\".\n\nAbstract: Paper abstract\n\nPlease respond the invitation clicking the following link:\n\nhttps://openreview.net/invitation?id=TMLRE/Reviewers/-/Assignment_Recruitment&user=harold@hotmail.com&key=5702b25b819d98308e99b9784e37d7327e511efbd618e44df194ccefc6dcc96c&submission_id={submission.id}&inviter=~Samy_Bengio1\n\nThanks,\n\nTMLRE Paper{submission.number} Action Editor {joelle_paper13_anon_group.id.split('_')[-1]}\nSamy Bengio\n\nPlease note that responding to this email will direct your reply to samy@bengio.com.\n'''

        invitation_url = re.search('https://.*\n', messages[0]['content']['text']).group(0).replace('https://openreview.net', 'http://localhost:3030').replace('&amp;', '&')[:-1]
        helpers.respond_invitation(selenium, request_page, invitation_url, accept=True)

        helpers.await_queue_edit(openreview_client, invitation='TMLRE/Reviewers/-/Assignment_Recruitment', count=2)

        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='harold@hotmail.com')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Pending Sign Up'         

        assignment_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_14, tail='harold@hotmail.com')
        assert len(assignment_edges) == 0

        messages = openreview_client.get_messages(to = 'harold@hotmail.com', subject = f'[TMLRE] Reviewers Invitation accepted for paper {submission.number}: Paper title 14, assignment pending')
        assert len(messages) == 1
        ...
        messages = openreview_client.get_messages(to = 'samy@bengio.com', subject = f'[TMLRE] Reviewers harold@hotmail.com accepted to review paper {submission.number}: Paper title 14, assignment pending')
        assert len(messages) == 1
        ...
        with pytest.raises(openreview.OpenReviewException, match=r'tail "javier@mailtwo.com" is member of TMLRE/Reviewers'):
            paper_assignment_edge = samy_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Invite_Assignment',
                signatures=[joelle_paper13_anon_group.id],
                head=note_id_14,
        ...
        with pytest.raises(openreview.OpenReviewException, match=r'Already invited as ~Rachel_TMLRE1'):
            paper_assignment_edge = samy_client.post_edge(openreview.api.Edge(invitation='TMLRE/Reviewers/-/Invite_Assignment',
                signatures=[joelle_paper13_anon_group.id],
                head=note_id_14,
                tail='melisa@mailten.com',
        ...
        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='~Harold_Red1')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Accepted'

        messages = openreview_client.get_messages(to = 'harold@hotmail.com', subject = f'[TMLRE] Reviewer Assignment confirmed for paper {submission.number}: Paper title 14')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi Harold Red,\nThank you for accepting the invitation to review the paper number: {submission.number}, title: Paper title 14.\n\nPlease go to the Tasks page and check your TMLRE pending tasks: https://openreview.net/tasks\n\nIf you would like to change your decision, please click the Decline link in the previous invitation email.\n\nOpenReview Team\n\nPlease note that responding to this email will direct your reply to tmlre@jmlr.org.\n'''

        messages = openreview_client.get_messages(to = 'samy@bengio.com', subject = f'[TMLRE] Reviewer Harold Red signed up and is assigned to paper {submission.number}: Paper title 14')
        assert len(messages) == 1
        ...
        assignment_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_14, tail='~Harold_Red1')
        helpers.await_queue_edit(openreview_client, edit_id=assignment_edges[0].id)
        ...
        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='~David_Belanger1')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Invitation Sent'         

        messages = openreview_client.get_messages(to = 'david@mailone.com', subject = '[TMLRE] Invitation to review paper titled "Paper title 14"')
        assert len(messages) == 1
        assert messages[0]['content']['text'] == f'''Hi David Belanger,\n\nYou were invited to review the paper number: {submission.number}, title: \"Paper title 14\".\n\nAbstract: Paper abstract\n\nPlease respond the invitation clicking the following link:\n\nhttps://openreview.net/invitation?id=TMLRE/Reviewers/-/Assignment_Recruitment&user=~David_Belanger1&key=1fb0276b3b49168f226d7d72a35ce85c60ad895f83a0e767b1a0c42a3fd9fd16&submission_id={submission.id}&inviter=~Samy_Bengio1\n\nThanks,\n\nTMLRE Paper{submission.number} Action Editor {joelle_paper13_anon_group.id.split('_')[-1]}\nSamy Bengio\n\nPlease note that responding to this email will direct your reply to samy@bengio.com.\n'''

        invitation_url = re.search('https://.*\n', messages[0]['content']['text']).group(0).replace('https://openreview.net', 'http://localhost:3030').replace('&amp;', '&')[:-1]
        helpers.respond_invitation(selenium, request_page, invitation_url, accept=True)

        helpers.await_queue_edit(openreview_client, invitation='TMLRE/Reviewers/-/Assignment_Recruitment', count=3)

        invite_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Invite_Assignment', head=note_id_14, tail='~David_Belanger1')
        assert len(invite_edges) == 1
        assert invite_edges[0].label == 'Accepted'         

        assignment_edges=openreview_client.get_edges(invitation='TMLRE/Reviewers/-/Assignment', head=note_id_14, tail='~David_Belanger1')
        assert len(assignment_edges) == 1
        ...
        helpers.await_queue_edit(openreview_client, edit_id='TMLRE/-/Preferred_Emails-0-0', count=3)

        ## Check preferred emails
        assert openreview_client.get_edges_count(invitation='TMLRE/-/Preferred_Emails') == 17        
