...
class InvitationBuilder(object):
    ...
    def set_super_review_invitation(self, journal):
        venue_id = journal.venue_id
        editors_in_chief_id = journal.get_editors_in_chief_id()
        paper_action_editors_id = journal.get_action_editors_id(number='${params.noteNumber}')
        paper_reviewers_id = journal.get_reviewers_id(number='${params.noteNumber}')
        paper_reviewers_anon_id = journal.get_reviewers_id(number='${params.noteNumber}', anon=True)
        paper_authors_id = journal.get_authors_id(number='${params.noteNumber}')

        review_invitation_id = journal.get_review_id()
        paper_review_invitation_id = journal.get_review_id(number='${params.noteNumber}')

        with open(os.path.join(os.path.dirname(__file__), 'process/review_process.py')) as f:
            paper_process = f.read()
            paper_process = paper_process.replace('openreview.journal.Journal()', f'openreview.journal.Journal(client, "{venue_id}", "{journal.secret_key}", contact_info="{journal.contact_info}", full_name="{journal.full_name}", short_name="{journal.short_name}")')
    ...
    def set_review_invitation(self, journal, note, duedate):
        ...
        self.client.post_invitation_edit_ex(invitations=journal.get_review_id(), params={ 'noteId': note.id, 'noteNumber': note.number, 'duedate': duedate }, signatures=[journal.venue_id], invitationId=journal.get_review_id(number=note.number))
