def process(client, edit, invitation):
    ...
    if not edit.note.content.get('pdf', {}).get('value') and not edit.note.content.get('beyond_pdf', {}).get('value'):
        raise openreview.OpenReviewException(f'You must upload a PDF or a Beyond PDF file for the submission.')
    
    if edit.note.content.get('pdf', {}).get('value') and edit.note.content.get('beyond_pdf', {}).get('value'):
