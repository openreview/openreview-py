def process_update(client, edge, invitation, existing_edge):
    ...
    if edge.ddate and edge.tail in group.members:
        ...
        client.post_message(journal.get_meta_invitation_id(), subject, recipients, message, venue_id, replyTo=assigned_action_editor.get_preferred_email())
    ...
    if not edge.ddate and edge.tail not in group.members:
        ...
        if official_reviewer:
            client.post_message(journal.get_meta_invitation_id(), subject, recipients, message, venue_id, parentGroup=group.id, replyTo=assigned_action_editor.get_preferred_email())
...
'''
        client.post_message(journal.get_meta_invitation_id(), subject, recipients, message, venue_id, ignoreRecipients=ignoreRecipients, parentGroup=group.id, replyTo=journal.contact_info)
