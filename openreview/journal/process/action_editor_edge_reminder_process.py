def process(client, invitation):
    ...
    task = "Reviewers Assignment"

    assignment_edges = client.get_edges(invitation=journal.get_reviewer_assignment_id(), head=submission.id)

    if len(assignment_edges) >= 3:
      return

    ## send email to actions editors
...
''',
    ...
    if date_index > 0:
      ## get preferred names
      action_editors = client.get_group(journal.get_action_editors_id(number=submission.number))
      profiles = openreview.tools.get_profiles(client, action_editors.members)
