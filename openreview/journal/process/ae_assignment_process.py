def process_update(client, edge, invitation, existing_edge):
    ...
    if edge.ddate and edge.tail in group.members:
        ...
        client.post_message(f'[{journal.short_name}] You have been unassigned from {journal.short_name} submission {note.number}: {note.content["title"]["value"]}', [edge.tail],
            ae_group.content['unassignment_email_template_script']['value'].format(
                short_name=journal.short_name,
                submission_number=note.number,
                submission_title=note.content['title']['value'],
                contact_info=journal.contact_info,
            ), parentGroup=group.id, replyTo=journal.contact_info, invitation=journal.get_meta_invitation_id(), signature=journal.venue_id, sender=journal.get_message_sender())

        client.remove_members_from_group(group.id, edge.tail)

        ## update assigned_action_editor if exists in the submission
        content = {}
        if ('assigned_action_editor' in note.content and
                note.content['assigned_action_editor']['value'] == edge.tail):
        ...
        if (journal.assigned_AE_venue_id == note.content['venueid']['value'] and
                ('assigned_action_editor' in content or
                 journal.settings.get('action_editor_paper_visibility') == 'assigned_only'
    ...
    if not edge.ddate and edge.tail not in group.members:
        ...
        client.post_message(f'[{journal.short_name}] Assignment to new {journal.short_name} submission {note.number}: {note.content["title"]["value"]}', [edge.tail],
            ae_group.content['assignment_email_template_script']['value'].format(
                short_name=journal.short_name,
                submission_number=note.number,
                submission_title=note.content['title']['value'],
        ...
        assigned_only = journal.settings.get('action_editor_paper_visibility') == 'assigned_only'
        ...
        if not assigned_only and (journal.is_action_editor_anonymous() or note.content.get('venueid', {}).get('value') in [journal.submitted_venue_id, journal.assigning_AE_venue_id, journal.assigned_AE_venue_id]):
        ...
        for eic in eics:
            ...
            if is_author:
                client.post_message(f'[{journal.short_name}] Attention: you\'ve been assigned a submission authored by an EIC', [edge.tail],
                    ae_group.content['eic_as_author_email_template_script']['value'].format(
                        short_name=journal.short_name,
                        contact_info=journal.contact_info,
