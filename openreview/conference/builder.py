...
class Conference(object):
    ...
    def __expire_invitations(self, name):
        invitations = tools.concurrent_get_invitations(self.client, regex = self.get_invitation_id(name, '.*'))
    ...
    def __create_review_rebuttal_stage(self):
        ...
        review_iterator = tools.concurrent_get_notes(self.client, invitation = invitation)
    ...
    def get_submissions(self, accepted = False, number=None, details = None, sort = None):
        ...
        notes = tools.concurrent_get_notes(self.client, invitation = invitation, number=number, details = details, sort = sort)
        if accepted:
            decisions = tools.concurrent_get_notes(self.client, invitation = self.get_invitation_id(self.decision_stage.name, '.*'))
    ...
    def get_withdrawn_submissions(self, details=None):
        ...
        return tools.concurrent_get_notes(self.client, invitation=invitation, details=details)
    ...
    def create_blind_submissions(self, hide_fields=[], under_submission=False, under_submission_readers=[]):
        ...
        for note in tqdm(tools.concurrent_get_notes(self.client, invitation=self.get_submission_id(), sort='number:asc'), desc='create_blind_submissions'):
    ...
    def setup_final_deadline_stage(self, force=False, hide_fields=[]):
        ...
        if not self.submission_stage.double_blind and not self.submission_stage.papers_released and not self.submission_stage.create_groups:
            ...
            for note in tqdm(tools.concurrent_get_notes(self.client, invitation=self.get_submission_id(), sort='number:asc'), desc='set_final_readers'):
...
Program Chairs
    ...
    def remind_registration_stage(self, subject, message, committee_id):
        ...
        confirmations = {c.tauthor: c for c in tools.concurrent_get_notes(self.client, invitation=self.get_registration_id(committee_id))}
    ...
    def post_decision_stage(self, reveal_all_authors=False, reveal_authors_accepted=False, release_all_notes=False, release_notes_accepted=False, decision_heading_map=None):
        ...
        decisions_by_forum = {n.forum: n for n in tools.concurrent_get_notes(self.client, invitation = self.get_invitation_id(self.decision_stage.name, '.*'))}
