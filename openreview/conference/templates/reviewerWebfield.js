...
var getCustomLoad = function(userIds) {
  return Webfield.get('/edges', { invitation: CUSTOM_MAX_PAPERS_ID, tail: user.id })
    .then(function(result) {
      ...
      return Webfield.get('/notes', { invitation: CUSTOM_LOAD_INVITATION, select: 'content.reviewer_load,content.user,content.reduced_quota' })
      .then(function(result) {
        if (!result.notes || !result.notes.length) {
          return REVIEW_LOAD;
        }
        if (result.notes.length === 1) {
          return result.notes[0].content.reviewer_load || result.notes[0].content.reduced_quota;
        } else {
          ...
          return loads.length ? (loads[0].content.reviewer_load || loads[0].content.reduced_quota ) : REVIEW_LOAD;
        }
      });
    })
