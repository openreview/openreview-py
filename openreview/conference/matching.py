...
def _get_profiles(client, ids_or_emails, with_publications=False):
    ...
    if with_publications:
        for profile in profiles:
            profile.content['publications'] = tools.concurrent_get_notes(client, content={'authorids': profile.id})
...
class Matching(object):
    ...
    def append_note_conflicts(self, profile_id, build_conflicts=None):
        ...
        submissions = openreview.tools.concurrent_get_notes(
    ...
    def _build_subject_area_scores(self, submissions):
        ...
        edges = []
        user_subject_areas = openreview.tools.concurrent_get_notes(
            self.client,
    ...
    def _build_custom_max_papers(self, user_profiles):
        ...
        reduced_loads = {}
        reduced_load_notes = openreview.tools.concurrent_get_notes(self.client, invitation=self.conference.get_invitation_id('Reduced_Load', prefix = self.match_group.id), sort='tcdate:asc')
    ...
    def deploy_acs(self, assignment_title, overwrite):
        ...
        papers = openreview.tools.concurrent_get_notes(self.client, invitation=self.conference.get_blind_submission_id())
    ...
    def deploy_sac_assignments(self, assignment_title, overwrite):
        ...
        ac_groups = {g.id:g for g in openreview.tools.concurrent_get_groups(self.client, regex=self.conference.get_area_chairs_id('.*'))}
