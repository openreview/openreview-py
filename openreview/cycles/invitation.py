...
'''
    ...
    def set_review_invitation(self):
        ...
        with open(process_file) as f:
            ...
            file_content = file_content.replace("PAPER_ACTION_EDITORS_ID = ''", f"PAPER_ACTION_EDITORS_ID = '{self.venue.get_action_editors_id('{number}')}'")

            if self.venue.use_action_editors:
                file_content = file_content.replace("USE_ACTION_EDITORS = False", "USE_ACTION_EDITORS = True")

            if review_stage.email_pcs:
                file_content = file_content.replace("EDITORS_IN_CHIEF_ID = ''", f"EDITORS_IN_CHIEF_ID = '{self.venue.get_editors_in_chief_id()}'")
...
''',
        ...
        invitation = Invitation(id=meta_review_invitation_id,
            ...
            edit={
                ...
                'invitation': {
                    ...
                    'readers': ['everyone'],
                    'writers': [venue_id],
                    'invitees': [venue_id, self.venue.get_action_editors_id(number='${3/content/noteNumber/value}')],
                    'maxReplies': 1,
                    'cdate': meta_review_cdate,
                    'edit': {
    ...
    def set_recruitment_invitation(self, committee_name, options):
        ...
        if not options.get('allow_overlap_official_committee'):
            if committee_name == venue.reviewers_name and venue.use_action_editors:
                invitation_content['overlap_committee_name'] = { 'value': venue.action_editors_name }
                invitation_content['overlap_committee_id'] = { 'value': venue.get_action_editors_id() }
            elif committee_name == venue.action_editors_name:
    ...
    def set_bid_invitations(self):
        ...
        for bid_stage in venue.bid_stages:
            ...
            if match_group_id == venue.get_senior_action_editors_id():
                head = {
                    'param': {
                        'type': 'profile',
                        'inGroup': venue.get_action_editors_id()
                    }
                }
    ...
    def set_official_comment_invitation(self):
        ...
        with open(process_file) as f:
            ...
            if self.venue.use_action_editors:
                process_content = process_content.replace("PAPER_ACTION_EDITORS_ID = ''", f"PAPER_ACTION_EDITORS_ID = '{self.venue.get_action_editors_id('{number}')}'")

            if self.venue.use_senior_action_editors:
                process_content = process_content.replace("PAPER_SENIOR_ACTION_EDITORS_ID = ''", f"PAPER_SENIOR_ACTION_EDITORS_ID = '{self.venue.get_senior_action_editors_id('{number}')}'")

            if comment_stage.email_pcs:
                process_content = process_content.replace("EDITORS_IN_CHIEF_ID = ''", f"EDITORS_IN_CHIEF_ID = '{self.venue.get_editors_in_chief_id()}'")

        process_file = os.path.join(os.path.dirname(__file__), 'process/comment_pre_process.py')
        with open(process_file) as f:
            pre_process_content = f.read()
            mandatory_readers = [self.venue.get_editors_in_chief_id()]
            if self.venue.use_senior_action_editors:
                mandatory_readers.append(self.venue.get_senior_action_editors_id(number='{number}'))
        ...
        invitation = Invitation(id=decision_invitation_id,
            ...
            edit={
                ...
                'invitation': {
                    ...
                    'readers': ['everyone'],
                    'writers': [venue_id],
                    'invitees': [self.venue.get_editors_in_chief(), self.venue.support_user],
                    'maxReplies': 1,
                    'minReplies': 1,
                    'cdate': decision_cdate,
                    'process': '''def process(client, edit, invitation):
...
''',
                    'edit': {
                        'signatures': [self.venue.get_editors_in_chief_id()],
    ...
    funcs['process'](client, edit, invitation)''',
        ...
        with open(process_file) as f:
            ...
            file_content = file_content.replace("DESK_REJECTED_INVITATION_ID = ''", f"DESK_REJECTED_INVITATION_ID = '{self.venue.get_desk_rejected_id()}'")
            file_content = file_content.replace("SHORT_NAME = ''", f'SHORT_NAME = "{self.venue.short_name}"')
            file_content = file_content.replace("COMMITTEE = []", f'COMMITTEE = {json.dumps(self.venue.get_participants(number="{number}", with_editors_in_chief=True, with_authors=True))}')
    ...
    def set_assignment_invitation(self, committee_id):
        ...
        is_action_editor = committee_id == venue.get_action_editors_id()
        is_senior_action_edtior = committee_id == venue.get_senior_action_edtiors_id()
        ...
        if is_action_editor:
            ...
            anon_regex = venue.get_action_editors_id('{number}', '.*', True)
            paper_group_id = venue.get_action_editors_id(number='{number}')
            group_name = venue.get_action_editors_name(pretty=True)
