...
class Matching(object):
    ...
    def __init__(self, venue, match_group, alternate_matching_group=None):
        ...
        self.is_action_editor = venue.get_action_editors_id() == match_group.id
        self.is_senior_action_editor = venue.get_senior_action_editors_id() == match_group.id
        self.is_ethics_reviewer = venue.get_ethics_reviewers_id() == match_group.id
        self.should_read_by_action_editor = venue.get_reviewers_id() == match_group.id and venue.use_action_editors
    ...
    def _get_edge_readers(self, tail):
        ...
        if self.should_read_by_action_editor:
            if self.venue.use_senior_action_editors:
                readers.append(self.venue.get_senior_action_editors_id())
            readers.append(self.venue.get_action_editors_id())
        readers.append(tail)
        return readers
    ...
    def _create_edge_invitation(self, edge_id, any_tail=False, default_label=None):
        ...
        edge_signatures = [venue_id + '$', venue.get_editors_in_chief_id()]
        edge_nonreaders = []
        ...
        if self.is_reviewer:
            if venue.use_senior_action_editors:
                edge_readers.append(venue.get_senior_action_editors_id(number=paper_number))
                invitation_readers.append(venue.get_senior_action_editors_id())
            if venue.use_area_chairs:
                edge_readers.append(venue.get_action_editors_id(number=paper_number))
                invitation_readers.append(venue.get_action_editors_id())

            if is_assignment_invitation:
                if venue.use_senior_action_editors:
                    edge_invitees.append(venue.get_senior_action_editors_id())
                    edge_writers.append(venue.get_senior_action_editors_id(number=paper_number))
                    edge_signatures.append(venue.get_senior_action_editors_id(number=paper_num_signatures))
                if venue.use_action_editors:
                    edge_invitees.append(venue.get_action_editors_id())
                    edge_writers.append(venue.get_action_editors_id(number=paper_number))
                    edge_signatures.append(venue.get_action_editors_id(number=paper_num_signatures))

                edge_nonreaders = [venue.get_authors_id(number=paper_number)]

        if self.is_action_editor:
            if venue.use_senior_action_editors:
                edge_readers.append(venue.get_senior_action_editors_id(number=paper_number))
                invitation_readers.append(venue.get_senior_action_editors_id())
        ...
        if self.is_ethics_reviewer:
            if venue.use_ethics_editors:
                edge_readers.append(venue.get_ethics_editors_id())
                invitation_readers.append(venue.get_ethics_editors_id())

            if is_assignment_invitation:
                if venue.use_ethics_editors:
                    edge_invitees.append(venue.get_ethics_editors_id())
                    edge_writers.append(venue.get_ethics_editors_id())
                    edge_signatures.append(venue.get_ethics_editors_id())
            ...
            edge = {
                ...
                'signatures': {
                    'param': { 
                        ...
                        'default': [venue.get_editors_in_chief_id()]
                    }
                },
                'head': edge_head,
                'tail': edge_tail,
    ...
    def setup(self, compute_affinity_scores=False, compute_conflicts=False):
        ...
        if not self.is_senior_action_editor:
    ...
    def deploy_assignments(self, assignment_title, overwrite):
        ...
        if role_name in venue.action_editor_roles:
            reviewer_name = venue.action_editors_name
    ...
    def deploy_sae_assignments(self, assignment_title, overwrite):
        ...
        ae_groups = {g.id:g for g in client.get_all_groups(regex=venue.get_action_editors_id('.*'))}
        ...
        for paper in tqdm(papers):
            ...
            ae_group_id=venue.get_action_editors_id(paper.number)
            ae_group=ae_groups.get(ae_group_id)
            if ae_group:
                if len(ae_group.members) == 0:
                    raise openreview.OpenReviewException('AC assignments must be deployed first')

                for ae in ae_group.members:
                    sae_assignments = proposed_assignment_edges.get(ae, [])

                    for sae_assignment in sae_assignments:
                        sae=sae_assignment['tail']
                        sae_group_id=ae_group.id.replace(venue.action_editors_name, venue.senior_action_editors_name)
                        sae_group=client.get_group(sae_group_id)
                        if overwrite:
                            sae_group.members=[]
                        sae_group.members.append(sae)
        ...
        for head, sae_assignments in proposed_assignment_edges.items():
            for sae_assignment in sae_assignments:
                assignment_edges.append(Edge(
                    invitation=assignment_invitation_id,
                    head=head,
                    tail=sae_assignment['tail'],
                    readers=sae_assignment['readers'],
                    writers=sae_assignment['writers'],
                    signatures=sae_assignment['signatures'],
                    weight=sae_assignment.get('weight')
                ))
    ...
    def deploy(self, assignment_title, overwrite=False, enable_reviewer_reassignment=False):
        ...
        if self.match_group.id == self.venue.get_senior_action_editors_id():
            self.deploy_sae_assignments(assignment_title, overwrite)
        else:
