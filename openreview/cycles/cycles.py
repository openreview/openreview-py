...
class Cycles(object):
    ...
    def __init__(self, client, venue_id):
        ...
        self.action_editor_roles = ['Action_Edtior']
        self.senior_action_edtior_roles = ['Senior_Action_Edtior']        
        self.action_editors_name = 'Action_Editors'
        self.senior_action_editors_name = 'Senior_Action_Editors'
        self.ethics_editors_name = 'Ethics_Editors'
        self.ethics_reviewers_name = 'Ethics_Reviewers'
        self.authors_name = 'Authors'
        self.use_ethics_editors = False
        ...
        self.use_action_editors = False
        self.use_senior_action_editors = False
        self.use_ethics_editors = False
        ...
        self.action_edtior_identity_readers = []
        self.senior_action_edtior_identity_readers = []        
    ...
    def get_committee_names(self):
        ...
        if self.use_action_editors:
            committee.append(self.action_editors_name)

        if self.use_senior_action_editors:
            committee.append(self.senior_action_editors_name)
    ...
    def get_roles(self):
        ...
        if self.use_action_editors:
            roles = self.reviewer_roles + [self.action_editors_name]
        if self.use_senior_action_editors:
            roles = roles + [self.senior_action_editors_name]
        return roles
    ...
    def get_committee(self, number = None, submitted_reviewers = False, with_authors = False):
        ...
        if self.use_action_editors:
            committee.append(self.get_action_editors_id(number))

        if self.use_senior_action_editors:
            committee.append(self.get_senior_action_editors_id(number))

        committee.append(self.get_editors_in_chief_id())
    ...
    def get_action_editors_name(self, pretty=True):
        if pretty:
            name=self.action_editors_name.replace('_', ' ')
            return name[:-1] if name.endswith('s') else name
        return self.action_editors_name
    ...
    def get_editors_in_chief_id(self):
        return self.get_committee_id(self.editors_in_chief_name)

    def get_action_editors_id(self, number = None, anon=False):
        return self.get_committee_id('Action_Editor_.*' if anon else self.action_editors_name, number)

    ## Compatibility with Conference, refactor conference references to use get_area_chairs_id
    def get_anon_action_editor_id(self, number, anon_id):
        return self.get_action_editors_id(number, True)

    def get_senior_action_editors_id(self, number = None):
        return self.get_committee_id(self.senior_action_editors_name, number)

    def get_ethics_editors_id(self, number = None):
        return self.get_committee_id(self.ethics_editors_name, number)
    ...
    def get_participants(self, number=None, with_editors_in_chief=False, with_authors=False):
        committee = []
        if with_editors_in_chief:
            committee.append(self.get_editors_in_chief_id())
        if self.use_senior_action_editors:
            committee.append(self.get_senior_action_editors_id(number))
        if self.use_action_editors:
    ...
    def setup(self, editors_in_chief_ids=[]):
    
        self.invitation_builder.set_meta_invitation()

        self.group_builder.create_venue_group()

        self.group_builder.create_editors_in_chief_group(editors_in_chief_ids)
        ...
        if self.use_action_editors:
            self.group_builder.create_action_editors_group()

        if self.use_senior_action_editors:
            self.group_builder.create_senior_action_editors_group()            
...
'''
        if self.request_form_id:
            ...
            status_note = openreview.Note(
                ...
                readers=[self.get_editors_in_chief_id(), self.support_user],
                writers=[],
    ...
    def setup_committee_matching(self, committee_id=None, compute_affinity_scores=False, compute_conflicts=False, alternate_matching_group=None):
        if committee_id is None:
            committee_id=self.get_reviewers_id()
        if self.use_senior_action_editors and committee_id == self.get_senior_action_editors_id() and not alternate_matching_group:
            alternate_matching_group = self.get_action_editors_id()
