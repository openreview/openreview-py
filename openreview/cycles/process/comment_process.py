def process(client, edit, invitation):
    ...
    PAPER_SENIOR_ACTION_EDITORS_ID = ''
    ...
    PAPER_ACTION_EDITORS_ID = PAPER_ACTION_EDITORS_ID.replace('{number}', str(submission.number))
    PAPER_SENIOR_ACTION_EDITORS_ID = PAPER_SENIOR_ACTION_EDITORS_ID.replace('{number}', str(submission.number))
...
To view the comment, click here: https://openreview.net/forum?id={submission.id}&noteId={comment.id}'''
    ...
    if EDITORS_IN_CHIEF_ID and (EDITORS_IN_CHIEF_ID in comment.readers or 'everyone' in comment.readers):
        client.post_message(
            recipients=[EDITORS_IN_CHIEF_ID],
    ...
    email_SAE = len(comment.readers)==3 and PAPER_SENIOR_ACTION_EDITORS_ID in comment.readers and EDITORS_IN_CHIEF_ID in comment.readers
    if PAPER_SENIOR_ACTION_EDITORS_ID and email_SAE:
        client.post_message(
            recipients=[PAPER_SENIOR_ACTION_EDITORS_ID],
            ignoreRecipients = ignore_groups,
    ...
    if PAPER_ACTION_EDITORS_ID and (PAPER_ACTION_EDITORS_ID in comment.readers or 'everyone' in comment.readers):
        client.post_message(
            recipients=[PAPER_ACTION_EDITORS_ID],
            ignoreRecipients=ignore_groups,
            subject=f'''{SHORT_PHRASE} {pretty_signature} commented on a paper in your area. Paper Number: {submission.number}, Paper Title: "{submission.content['title']['value']}"''',
