...
The comment can be viewed on OpenReview here: https://openreview.net/forum?id={forum}&noteId={id}'''.format(action=action, title=submission.content['title'], forum=note.forum, id=note.id),
    ...
    client.post_message(subject='[Agora/COVID-19] Your comment has been {action} on your assigned article titled "{title}"'.format(action=action, title=submission.content['title']),
        recipients=note.signatures,
        message='''Your comment has been {action} to your assigned article titled "{title}".
