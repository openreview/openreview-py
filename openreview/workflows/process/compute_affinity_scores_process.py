def process(client, invitation):
    ...
    prefix = venue_id + '/'
    comment = f'The process "{invitation.id.split(prefix)[-1].replace("_", " ").replace("/-/", " ")}" has successfully completed. Please note that affinity scores could not be computed for users with no profile or no publications.'
    ...
    client.post_note_edit(
        ...
        note=openreview.api.Note(
            ...
            signatures=[venue_id],
            content={
                'title': { 'value': f'{committee_name.replace("_", " ").title()} Affinity Scores Computation Completed' },
