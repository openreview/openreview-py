...
var addBulkInviteRow = function(values) {
  ...
  $('#bulk-invite-recipient-rows').append(
    '<div class="bulk-invite-recipient-row row" style="margin-bottom: 8px;">' +
      '<div class="col-sm-4"><input class="form-control bulk-invite-openreviewid" type="text" placeholder="~OpenReview_ID" value="' + _.escape(values.openreviewid || '') + '"></div>' +
      '<div class="col-sm-4"><input class="form-control bulk-invite-email" type="email" placeholder="email@example.edu" value="' + _.escape(values.email || '') + '"></div>' +
      '<div class="col-sm-3"><input class="form-control bulk-invite-name" type="text" placeholder="Full Name" value="' + _.escape(values.name || '') + '"></div>' +
      '<div class="col-sm-1"><button type="button" class="btn btn-default js-remove-bulk-invite-row" aria-label="Remove invitee row">&times;</button></div>' +
    '</div>'
  );
};
...
var setBulkInviteRows = function(rows) {
  ...
  rows.forEach(function(row) { addBulkInviteRow(row); });
  if (!rows.length) addBulkInviteRow();
};
...
var bulkInviteRowsToInviteeDetails = function(rows) {
  return rows.map(function(row) {
    return row.openreviewid ? row.openreviewid : (row.name ? row.email + ', ' + row.name : row.email);
  }).join('\n');
};
...
var submitBulkInvite = function() {
  ...
  if (typeof confirm === 'function' && !confirm('Send ' + config.label + ' recruitment email to ' + recipientData.rows.length + ' invitee(s)?')) {
    return;
  }
  ...
  Promise.all(recipientData.rows.map(function(row) {
    ...
    setBulkInviteStatus('Recruitment messages created for ' + recipientData.rows.length + ' invitee(s).', false);
...
var renderBulkInviteTab = function(data) {
  jmlrBulkInviteTemplates = data.bulkInviteTemplates || {};
  $('#bulk-invite').html(
    '<div class="container" style="max-width: 980px;">' +
      '<h3>Bulk Invite</h3>' +
      '<p class="dark">Send JMLR recruitment invitations for reviewers or action editors. Add one invitee per row. Use <code>openreviewid</code> when available; otherwise enter <code>email</code> and optional <code>name</code>.</p>' +
      '<div class="form-group"><label for="bulk-invite-role">Role</label><select id="bulk-invite-role" class="form-control" style="max-width: 360px;"><option value="reviewer">Reviewers</option><option value="actionEditor">Action Editors</option></select><p id="bulk-invite-role-note" class="small text-muted mt-1"></p></div>' +
      '<div class="form-group"><label>Invitees</label><div class="row small text-muted" style="margin-bottom: 4px;"><div class="col-sm-4">openreviewid</div><div class="col-sm-4">email</div><div class="col-sm-3">name</div><div class="col-sm-1"></div></div><div id="bulk-invite-recipient-rows"></div><button id="bulk-invite-add-row" type="button" class="btn btn-default btn-sm" style="margin-top: 4px;">Add Invitee</button><p class="small text-muted" style="margin-top: 6px;">If <code>openreviewid</code> is present, email and name are ignored. If <code>openreviewid</code> is empty, email is required.</p></div>' +
      '<div class="form-group"><label for="bulk-invite-csv">Upload CSV invitees</label><input id="bulk-invite-csv" class="form-control" type="file" accept=".csv,text/csv"><p class="small text-muted">CSV files must use the header <code>openreviewid,email,name</code>. Blank, duplicate, and malformed rows are ignored in the preview.</p><p id="bulk-invite-preview" class="small text-muted">Preview: 0 accepted, 0 ignored.</p></div>' +
      '<div class="form-group"><label for="bulk-invite-subject">Email subject</label><input id="bulk-invite-subject" class="form-control" type="text"></div>' +
      '<div class="form-group"><label for="bulk-invite-content">Email content</label><textarea id="bulk-invite-content" class="form-control" rows="18"></textarea><p class="small text-muted">Keep {{accept_url}} and {{decline_url}} in the message.</p></div>' +
      '<p class="small text-muted">Recruitment response invitation: <code id="bulk-invite-invitation"></code></p>' +
      '<p><button id="bulk-invite-submit" type="button" class="btn btn-primary">Send Bulk Invite</button> <a class="btn btn-default" href="/forum?id=' + JOURNAL_REQUEST_ID + '">Open Journal Request Forum</a></p>' +
  ...
  $('#bulk-invite-add-row').off('click').on('click', function() { addBulkInviteRow(); updateBulkInvitePreview(); });
  ...
  $('#bulk-invite').off('input', '.bulk-invite-openreviewid,.bulk-invite-email,.bulk-invite-name').on('input', '.bulk-invite-openreviewid,.bulk-invite-email,.bulk-invite-name', function() { updateBulkInvitePreview(); });
