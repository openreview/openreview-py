var formatData = function(
    ...
    actionEditorStatusById[actionEditor.id] = {
      ...
      summary: {
        ...
        status: {
	      ...
	      availabilityData: getAvailabilityData(
	        ACTION_EDITORS_AVAILABILITY_ID,
	        ACTION_EDITOR_ID,
	        actionEditor.id,
	        availabilityEdge,
	        true
  ...
  submissions.forEach(function(submission) {
    ...
    if ((!actionEditor.id || actionEditor.id === 'No Action Editor') && activeActionEditorAssignmentEdge) {
      actionEditor = {
        id: activeActionEditorAssignmentEdge.tail,
        name: activeActionEditorAssignmentEdge.tail
      };
    }
    var paperStatus = getContentValue(submission.content, 'venue') || getContentValue(submission.content, 'venueid') || '';
    while (paperStatus && typeof paperStatus === 'object' && paperStatus.value !== undefined) {
      paperStatus = paperStatus.value;
    }
    if (paperStatus && typeof paperStatus === 'object') {
      paperStatus = getContentValue(submission.content, 'venueid') || '';
    }
    while (paperStatus && typeof paperStatus === 'object' && paperStatus.value !== undefined) {
      paperStatus = paperStatus.value;
    }
