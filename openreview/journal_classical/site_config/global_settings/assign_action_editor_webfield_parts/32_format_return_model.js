    ...
    paperStatusRows.push({
      ...
      actionEditorProgressData: {
        ...
        actionEditorAssignmentEdge: actionEditor.id ? (actionEditorAssignmentEdgesByHead[submission.id] || []).find(function(edge) { return edge.tail === actionEditor.id; }) || activeActionEditorAssignmentEdge : activeActionEditorAssignmentEdge,
  ...
  var pendingTaskRows = submittedStatusRows.filter(function(row) {
    return !row.actionEditorProgressData.actionEditor || !row.actionEditorProgressData.actionEditor.id || row.actionEditorProgressData.actionEditor.id === 'No Action Editor';
  });
  ...
  return applyConsoleModel({
    ...
    roleContextGroups: {
      editorsInChief: editorsInChiefGroup
    },
    bulkInviteTemplates: {
      reviewer: {
