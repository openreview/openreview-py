...
To view the comment, click here: https://openreview.net/forum?id={submission.id}&noteId={comment.id}'''
    ...
    email_program_chairs = email_pcs or (domain.get_content_value('direct_comment_email_pcs') and len(comment.readers) == minimum_number_of_readers)
    ...
    if email_area_chairs and paper_area_chairs_group and (paper_area_chairs_id in comment.readers or 'everyone' in comment.readers):
        client.post_message(
            invitation=meta_invitation_id,
