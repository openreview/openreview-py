...
def get_invitations_with_date_processes(client, venue_id):
    """
    Get all the invitations of a venue that have scheduled date process functions.

    :param client: API client used to read the invitations.
    :type client: openreview.api.OpenReviewClient
    :param venue_id: The venue id whose invitations should be returned.
    :type venue_id: str

    :return: List of invitations that have scheduled date process functions.
    :rtype: list[openreview.api.Invitation]
    """
    return [
        invitation for invitation in client.get_all_invitations(prefix=venue_id, domain=venue_id)
        if invitation.date_processes
    ]

def expire_date_processes(client, venue_id, invitations=None):
    """
    Disable the scheduled date process functions of a venue by deleting the invitations
    that have them.

    The invitations are deleted (not expired) so their original ``cdate``/``expdate`` are
    preserved -- expiring an invitation whose ``cdate`` is in the future would violate the
    ``cdate <= expdate`` constraint. Deleting also removes the scheduled processes. The
    deleted invitations can be brought back, and their date processes re-scheduled, with
    :func:`enable_date_processes` (for example after renaming a venue, whose rename carries
    the trashed invitations to the new domain but does not re-schedule their date processes).

    :param client: API client used to read and update the invitations.
    :type client: openreview.api.OpenReviewClient
    :param venue_id: The venue id whose invitations should be disabled.
    :type venue_id: str
    :param invitations: Invitations to disable. If not provided, they are looked up with
        :func:`get_invitations_with_date_processes`.
    :type invitations: list[openreview.api.Invitation], optional

    :return: The list of invitations that were disabled.
    :rtype: list[openreview.api.Invitation]
    """
    domain = client.get_group(venue_id)
    meta_invitation_id = domain.content['meta_invitation_id']['value']
    now = datetime_millis(datetime.datetime.now())

    if invitations is None:
        invitations = get_invitations_with_date_processes(client, venue_id)
...
def enable_date_processes(client, old_venue_id, new_venue_id, invitations):
    """
    Re-enable the date process functions of invitations that were disabled with
    :func:`expire_date_processes`, under the new venue domain.

    Each invitation is un-deleted at its new domain id, which re-schedules its date
    process functions under the new domain.

    :param client: API client used to update the invitations.
    :type client: openreview.api.OpenReviewClient
    :param old_venue_id: The venue id the invitations were captured from.
    :type old_venue_id: str
    :param new_venue_id: The venue id the invitations now live under (after the rename).
    :type new_venue_id: str
    :param invitations: Invitations captured by :func:`expire_date_processes`.
    :type invitations: list[openreview.api.Invitation]
    """
    domain = client.get_group(new_venue_id)
    meta_invitation_id = domain.content['meta_invitation_id']['value']

    for invitation in invitations:
        new_id = invitation.id.replace(old_venue_id, new_venue_id, 1)
        client.post_invitation_edit(
            invitations=meta_invitation_id,
            readers=[new_venue_id],
            writers=[new_venue_id],
            signatures=[new_venue_id],
            invitation=openreview.api.Invitation(
                id=new_id,
                ddate={ 'delete': True },
                signatures=[new_venue_id]
